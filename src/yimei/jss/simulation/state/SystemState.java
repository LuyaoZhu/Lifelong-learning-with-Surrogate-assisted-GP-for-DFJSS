...
public class SystemState {
    ...
    private List<Batch> batchesInSystem;
    private List<Batch> batchesCompleted;

    protected DynamicSimulation dynamicSimulation;

    public SystemState(double clockTime, List<WorkCenter> workCenters,
                       List<Job> jobsInSystem, List<Batch> batchesInSystem, List<Job> jobsCompleted, List<Batch> batchesCompleted) {
        this.clockTime = clockTime;
        this.workCenters = workCenters;
        this.jobsInSystem = jobsInSystem;
        this.batchesInSystem = batchesInSystem;
        this.jobsCompleted = jobsCompleted;
        this.batchesCompleted = batchesCompleted;
    }

    public SystemState(double clockTime) {
        this(clockTime, new ArrayList<>(), new LinkedList<>(), new LinkedList<>(), new ArrayList<>(), new ArrayList<>());
    }
    ...
    public List<Batch> getBatchesInSystem() {
        return batchesInSystem;
    }
    ...
    public List<Batch> getBatchesCompleted() {
    ...
    public void addBatchToSystem(Batch batch) {
        batchesInSystem.add(batch);
    }
...
//        }
    ...
    public void removeBatchFromSystem(Batch batch) {
        batchesInSystem.remove(batch);
//        if (jobsInSystem.size() == 0) {
//            if (!verifyRestrictionsMet(jobsCompleted)) {
//                //System.out.println("Still problems with machine allocation");
//            }
...
//        }
    ...
    public void addCompletedBatch(Batch batch) {
        batchesCompleted.add(batch);
...
//        }
    ...
    public double getMaxWorkLoadInSystem() {
        double maxWorkLoad = workCenters.get(0).getWorkInQueue();
        for (int j = 1; j < workCenters.size(); j++) {
            if (workCenters.get(j).getWorkInQueue() > maxWorkLoad)
                maxWorkLoad = workCenters.get(j).getWorkInQueue();
        }
        return maxWorkLoad;
    }

    public double getMinWorkLoadInSystem() {
        double minWorkLoad = workCenters.get(0).getWorkInQueue();
        for (int j = 1; j < workCenters.size(); j++) {
            if (workCenters.get(j).getWorkInQueue() < minWorkLoad)
                minWorkLoad = workCenters.get(j).getWorkInQueue();
        }
        return minWorkLoad;
    }
...
//    }
    ...
    public SystemState clone() {
        ...
        return new SystemState(clockTime, clonedWCs,
                new LinkedList<>(), new LinkedList<>(), new ArrayList<>(), new ArrayList<>());
    }
