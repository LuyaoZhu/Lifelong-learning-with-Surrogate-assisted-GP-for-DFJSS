...
//            simulation.addEvent(new ProcessStartEvent(p));
            new ProcessStartEvent(p).trigger(simulation);       //directly trigger this event
        }
    }
    ...
    public String toString() {
        return String.format("%.1f: batch %d job %d op %d visits.\n",
                time, operationOption.getJob().batch.getId(),operationOption.getJob().getId(), operationOption.getOperation().getId());
    }
