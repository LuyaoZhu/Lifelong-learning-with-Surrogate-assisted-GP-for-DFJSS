    ...
    public void trigger(Simulation simulation) {
        ...
        if (!workCenter.getQueue().isEmpty()) {
            ...
            OperationOption dispatchedOp =
                    simulation.getDefaultSequencingRule().priorOperation(sequencingDecisionSituation);
...
//            simulation.addEvent(new ProcessStartEvent(nextP));
            new ProcessStartEvent(nextP).trigger(simulation);
        }
        //----------------routing--------------------Luyao
        //set ready time to all operationOptions, to keep the terminal (OWT) accurate
        if(!(process.getOperationOption().getOperation().getNext() == null)) {
            for (OperationOption operationOption : process.getOperationOption().getOperation().getNext().getOperationOptions()) {
                operationOption.setReadyTime(time);
            }
        }

        OperationOption nextOp = process.getOperationOption().getNext(simulation.getSystemState(),
                simulation.getDefaultRoutingRule());
        if(simulation.getNumBatchesArrived() > simulation.getWarmupBatches()+1){
        ...
        if (nextOp == null) {
            ...
            job.getBatch().completedJobs++;
            simulation.completeJob(job);

            process.getOperationOption().getJob().operationRemaining = 0;
            process.getOperationOption().getJob().workLoadRemaining = 0;
...
//            simulation.addEvent(new OperationVisitEvent(time, nextOp));
            ...
            new OperationVisitEvent(time, nextOp).trigger(simulation);

            process.getOperationOption().getJob().operationRemaining --;
            process.getOperationOption().getJob().workLoadRemaining -= process.getOperationOption().getProcTime();
        }
    }
...
*/
    ...
    public String toString() {
        return String.format("%.1f: batch %d job %d op %d finished on work center %d.\n",
                time,
                process.getOperationOption().getJob().batch.getId(),
