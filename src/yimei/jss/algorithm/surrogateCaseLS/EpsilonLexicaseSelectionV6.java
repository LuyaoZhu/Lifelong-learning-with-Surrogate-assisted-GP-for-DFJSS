 ...
 */
public class EpsilonLexicaseSelectionV6 extends TournamentSelection {
...
//        if (((GPRuleEvolutionStateSavedSurrogateCasesV6) state).finalSelectedCasesIndex.size() < 15) {
...
//        int s = state.parameters.getIntWithDefault(new Parameter("candidate"), null, 800); // the number of candidates
        int s = (int)(0.8 * state.parameters.getIntWithDefault(new Parameter("pop.subpop.0.size"), null, 600)); // the number of candidates
...
//        System.out.println("TourIndividuals' size has not arrive at the set size in advance.");
    ...
    public List<Integer> getRandomSequenceInstance(EvolutionState state, int thread) {
        ...
        for (int i = 0; i < ((GPRuleEvolutionStateSavedSurrogateCasesV6) state).finalSelectedCasesIndex.size(); i++) {
            Instance.add(((GPRuleEvolutionStateSavedSurrogateCasesV6) state).finalSelectedCasesIndex.get(i));
        }
