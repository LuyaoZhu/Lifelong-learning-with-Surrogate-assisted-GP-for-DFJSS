...
//    int casesNum;
    ...
    public int produce(final int subpopulation, final EvolutionState state, final int thread) {
        ...
        ((GPRuleEvolutionStateSavedSurrogateCases) state).parentSelectionNum++;

//        if (state.generation < 200) {
        if (((GPRuleEvolutionStateSavedSurrogateCases) state).finalSelectedCasesIndex.size() < 5) {
...
/*            for (Map.Entry<List<Integer>, List<Integer>> pcToIndex : ((GPRuleEvolutionStateSavedSurrogateCases) state).pcToIndexMap.entrySet()) {

                if (pcToIndex.getValue().size() == 1) {
                    if (oldinds[pcToIndex.getValue().get(0)].fitness.fitness() < Double.MAX_VALUE)
                        allIndividualsIndex.add(pcToIndex.getValue().get(0));
                } else {
                    int randomIndex = state.random[0].nextInt(pcToIndex.getValue().size());
                    if (oldinds[pcToIndex.getValue().get(randomIndex)].fitness.fitness() < Double.MAX_VALUE)
                        allIndividualsIndex.add(pcToIndex.getValue().get(randomIndex));
                }
            }*/

            //randomly remove 20% inds
//            Collections.shuffle(allIndividualsIndex, new Random(608));
//            for (int i = 0; i < allIndividualsIndex.size() * 0.2; i++) {
//                allIndividualsIndex.remove(0);
//            }

//            Collections.shuffle(allIndividualsIndex, new Random(818));

//            for (int i = 0; i < allIndividualsIndex.size(); i++) {
//                allIndividuals.add(oldinds[allIndividualsIndex.get(i)]);
//            }

            //now, to increase the diversity of selected individuals, we remove 20% PC
/*            Set<Integer> randomNumbers = generateUniqueRandomNumbers((int) (allIndividuals.size() * 0.2), allIndividuals.size(), 12345);

            // 先转换成 List 以便排序
            List<Integer> indicesToRemove = new ArrayList<>(randomNumbers);
            // 反向排序，避免索引错位
            Collections.sort(indicesToRemove, Collections.reverseOrder());

            // 从后往前删除，防止索引错位
            for (int a : indicesToRemove) {
                allIndividualsIndex.remove(a);
                allIndividuals.remove(a);
            }*/

            //---------------end--------------------

            //first, delete individuals in the current generation (give the highest priority to the current case)
//            ((GPRuleEvolutionStateSavedSurrogateCases) state).finalSelectedCasesIndex.remove(state.generation);

/*            double random = state.random[thread].nextDouble();
//
            if (random < ((double) state.generation) / state.numGenerations) {
                ArrayList<Double> fitnessCurrentGen = new ArrayList<>();
                for (int ind = 0; ind < allIndividualsIndex.size(); ind++) {
                    fitnessCurrentGen.add(allIndividuals.get(ind).fitness.fitness());
                }
                double bestFitnessCurrentGen = Collections.min(fitnessCurrentGen);

                double epsilonCurrentGen = calculateEpsilon(fitnessCurrentGen);

                for (int ind = 0; ind < allIndividualsIndex.size(); ind++) {
                    if (allIndividuals.get(ind).fitness.fitness() > bestFitnessCurrentGen + epsilonCurrentGen) {
                        allIndividualsIndex.remove(ind);
                        allIndividuals.remove(ind);
                    }
                }
//                ((GPRuleEvolutionStateSavedSurrogateCases) state).finalSelectedCasesIndex.remove(Integer.valueOf(state.generation));
            }*/
            ...
            randomInstanceIndex.remove(Integer.valueOf(state.generation));
            randomInstanceIndex.add(0, state.generation);


/*            {
                List<Integer> reversedList = IntStream.rangeClosed(1, randomInstanceIndex.size()).mapToObj(i -> randomInstanceIndex.get(randomInstanceIndex.size() - i)).collect(Collectors.toList());
                List<Individual> allIndividualsCopy = new ArrayList<>();
                allIndividualsCopy.addAll(allIndividuals);
                List<Integer> allIndividualsIndexCopy = new ArrayList<>();
                allIndividualsIndexCopy.addAll(allIndividualsIndex);

                reverseCases(state, reversedList,allIndividualsCopy,allIndividualsIndexCopy);
...
/*            for (int i = 0; i < randomInstanceIndex.size(); i++) {
                ...
                if (allIndividuals.size() == 1) {
                    ...
                    ((GPIndividual) state.population.subpops[0].individuals[index]).fromTS = false;
                    return index;
                }

                int instanceIndex = randomInstanceIndex.get(i);

                // 计算所有候选个体在该测试案例上的 fitness
                List<Double> estimatedFitness = new ArrayList<>();
                for (int ind = 0; ind < allIndividualsIndex.size(); ind++) {
                ...
                // 记录个体索引及其 fitness
                List<Pair<Integer, Double>> individualFitness = new ArrayList<>();
                for (int ind = 0; ind < allIndividualsIndex.size(); ind++) {
                    individualFitness.add(new Pair<>(allIndividualsIndex.get(ind), estimatedFitness.get(ind)));
                }

                // 按照 fitness 进行排序
                individualFitness.sort(Comparator.comparingDouble(Pair::getValue));
                ...
                for (int j = 0; j < individualFitness.size(); j++) {
                    Pair<Integer, Double> current = individualFitness.get(j);
                    if (current.getValue() != previousFitness) {
                        rank += tiedCount;  // 跳过并列排名的个体
                        tiedCount = 1;
                    } else {
                        tiedCount++;
                    }
                    ranks.add(rank);
                    previousFitness = current.getValue();
                }

                // 计算 ε_rank
                double epsilonRank = calculateEpsilon(ranks);
                // 计算最佳排名
                double bestRank = ranks.get(0);  // 最佳排名（排名最小的个体）

                // 选择符合条件的个体
                List<Integer> selectedIndividuals = new ArrayList<>();
                for (int j = 0; j < ranks.size(); j++) {
                    if (ranks.get(j) <= (bestRank + epsilonRank)) {
                        selectedIndividuals.add(individualFitness.get(j).getKey());
                    }
                }

                // 如果只剩下一个个体，直接返回
                if (selectedIndividuals.size() == 1) {
                    return selectedIndividuals.get(0);
                }

                // 更新候选个体列表
                allIndividuals = new ArrayList<>();
                for (Integer selectedIndex : selectedIndividuals) {
                    allIndividuals.add(state.population.subpops[0].individuals[selectedIndex]);
                }
                allIndividualsIndex = new ArrayList<>(selectedIndividuals);
            }

// 如果处理完所有案例后仍有多个个体，随机选择一个
            int index = allIndividualsIndex.get(state.random[thread].nextInt(allIndividuals.size()));
            return index;*/
...
/*            for (int i = 0; i < randomInstanceIndex.size(); i++) {
            ...
            // 按照fitness排序
                individualFitness.sort((a, b) -> Double.compare(a.getValue(), b.getValue()));

                // 排名分配: 每个不同的 fitness 值得到一个排名
...
// 排名逻辑
                int totalIndividuals = ranks.size();
                int numToKeep = (int) Math.ceil(totalIndividuals * 0.30); // 保留前30%的个体

                // 获取前 30% 排名最好的个体
                List<Integer> selectedIndividuals = new ArrayList<>();
                for (int j = 0; j < ranks.size(); j++) {
                    if (ranks.get(j) <= numToKeep) {
                        selectedIndividuals.add(individualFitness.get(j).getKey());
                    }
                }
...
// 否则，更新 allIndividualsIndex 为选中的个体，并继续选择
// 获取个体对象
                allIndividuals = new ArrayList<>();
                for (Integer selectedIndex : selectedIndividuals) {
                    allIndividuals.add(state.population.subpops[0].individuals[selectedIndex]);
