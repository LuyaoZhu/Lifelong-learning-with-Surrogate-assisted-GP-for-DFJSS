...
public class GPRuleEvolutionStateSavedSurrogateCasesV3 extends GPRuleEvolutionState {
...
//            double fitness = fitnessList.stream().mapToDouble(d -> d).average().orElse(0.0);    // choose average
        ...
        if (generation >= switchGen) {
            ...
            for (int gen = generation-1; gen>=0; gen--){
                finalSelectedCasesIndex.add(gen);
            }
...
//            for (int instance:finalSelectedCasesIndex) {
                    ...
                    int num = 0;
                    for (int ind = 0; ind < this.population.subpops[0].individuals.length; ind++) {
                        int[] estimatedPC = indsCharListsMultiTreeCurrentGen[ind];
                        ...
                        if (dMin >= 3) { //use real evaluation to replace the estimation
                            ...
                            simulation.reset();
                            ((GPIndividual) population.subpops[0].individuals[ind]).caseFitness.add(ObjValue);
                    ...
                    System.out.println("The number of close sample in instance " + instance + " is " + num);
                } else {
                    for (int ind = 0; ind < this.population.subpops[0].individuals.length; ind++) {
                        ((GPIndividual) population.subpops[0].individuals[ind]).caseFitness.add(Double.MAX_VALUE);
                    }
                }
            }
            System.out.println("The number of extral real evaluations is " + realEvaluationNum);
            extraEvaluateNum.add(realEvaluationNum);
/*            List<Map.Entry<Integer, Double>> filtered = new ArrayList<>();
            for (int i = 0; i < averageDistance.size(); i++) {
                if (averageDistance.get(i) < 5) {
                    filtered.add(new AbstractMap.SimpleEntry<>(i, averageDistance.get(i)));
                }
            }
            // 按 value（即值）升序排序
            filtered.sort(Comparator.comparing(Map.Entry::getValue));
            // 取前 20 个并输出其索引
            for (int i = 0; i < Math.min(19, filtered.size()); i++) {
               finalSelectedCasesIndex.add(filtered.get(i).getKey());
            }*/
