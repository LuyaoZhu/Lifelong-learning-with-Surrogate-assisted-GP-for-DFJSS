 ...
 */
public class EpsilonLexicaseSelectionV5 extends TournamentSelection {
...
//        if (((GPRuleEvolutionStateSavedSurrogateCasesV5) state).finalSelectedCasesIndex.size() < 8) {
        if (state.generation < ((GPRuleEvolutionStateSavedSurrogateCasesV5) state).switchGen) {
...
//            ((GPIndividual)state.population.subpops[0].individuals[index]).fromTS = true;
            ...
            for (Map.Entry<List<Integer>, List<Integer>> pcToIndex : ((GPRuleEvolutionStateSavedSurrogateCasesV5) state).pcToIndexMap.entrySet()) {
...
//        System.out.println("TourIndividuals' size has not arrive at the set size in advance.");
    ...
    public List<Integer> getRandomSequenceInstance(EvolutionState state, int thread) {
        ...
        for (int i = 0; i < ((GPRuleEvolutionStateSavedSurrogateCasesV5) state).finalSelectedCasesIndex.size(); i++) {
            Instance.add(((GPRuleEvolutionStateSavedSurrogateCasesV5) state).finalSelectedCasesIndex.get(i));
        }
