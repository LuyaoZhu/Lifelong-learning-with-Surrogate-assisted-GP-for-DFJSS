...
/**
 * 1.use multipleLinearRegression to predict fitness instead of KNN
 *
 * @author yimei
 */

public class GPRuleEvolutionStateSavedSurrogateCasesV1 extends GPRuleEvolutionState {
        ...
        if (generation >= switchGen ) {
            ...
            for (int instance = 0; instance < surrogatePCEveryGen.size() - 1; instance++) {
                ...
                for (Map.Entry<List<Integer>, List<Integer>> pcToIndex : pcToIndexMap.entrySet()) {
                    ...
                    RealVector beta = MultiLinearRegression.fit(surrogatePCEveryGen.get(instance), surrogateFitnessEveryGen.get(instance));

                    double prediction = MultiLinearRegression.predict(beta,estimatedPC);

                        for (int indsIndex : pcToIndex.getValue()) {
                            ((GPIndividual) population.subpops[0].individuals[indsIndex]).caseFitness.add(prediction);
                        }
                }
            ...
            for(int gen = generation; gen >= generation-15; gen--) {
