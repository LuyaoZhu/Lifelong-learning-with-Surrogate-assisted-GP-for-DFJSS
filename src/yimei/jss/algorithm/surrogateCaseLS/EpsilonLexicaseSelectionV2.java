 ...
 */
public class EpsilonLexicaseSelectionV2 extends TournamentSelection {
...
//    int casesNum;
    ...
    public int produce(final int subpopulation, final EvolutionState state, final int thread) {
        ...
        ((GPRuleEvolutionStateSavedSurrogateCasesV2) state).parentSelectionNum++;

//        if (state.generation < 200) {
//        if (((GPRuleEvolutionStateSavedSurrogateCases) state).finalSelectedCasesIndex.size() < 5) {
        if (state.generation < ((GPRuleEvolutionStateSavedSurrogateCasesV2) state).switchGen) {
...
//            ((GPIndividual)state.population.subpops[0].individuals[index]).fromTS = true;
            ...
            List<Individual> allIndividuals = new ArrayList<>();
                List<Integer> allIndividualsIndex = ((GPRuleEvolutionStateSavedSurrogateCasesV2)state).topIndsIndex;
...
//        System.out.println("TourIndividuals' size has not arrive at the set size in advance.");
    ...
    public List<Integer> getRandomSequenceInstance(EvolutionState state, int thread) {
        ...
        for (int i = 0; i < ((GPRuleEvolutionStateSavedSurrogateCasesV2) state).finalSelectedCasesIndex.size(); i++) {
            Instance.add(((GPRuleEvolutionStateSavedSurrogateCasesV2) state).finalSelectedCasesIndex.get(i));
        }
