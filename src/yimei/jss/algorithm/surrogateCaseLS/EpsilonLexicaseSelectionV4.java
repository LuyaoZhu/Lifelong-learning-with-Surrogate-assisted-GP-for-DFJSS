 ...
 */
public class EpsilonLexicaseSelectionV4 extends TournamentSelection {
...
//        if (((GPRuleEvolutionStateSavedSurrogateCasesV4) state).finalSelectedCasesIndex.size() < 10) {
        if (state.generation < ((GPRuleEvolutionStateSavedSurrogateCasesV4) state).switchGen) {
...
//        System.out.println("TourIndividuals' size has not arrive at the set size in advance.");
    ...
    public List<Integer> getRandomSequenceInstance(EvolutionState state, int thread) {
        ...
        for (int i = 0; i < ((GPRuleEvolutionStateSavedSurrogateCasesV4) state).finalSelectedCasesIndex.size(); i++) {
            Instance.add(((GPRuleEvolutionStateSavedSurrogateCasesV4) state).finalSelectedCasesIndex.get(i));
        }
