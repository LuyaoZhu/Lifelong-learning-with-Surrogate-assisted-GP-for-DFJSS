...
public class GPRuleEvolutionStateSavedSurrogateCasesV5 extends GPRuleEvolutionState {
    ...
    public int evolve() {
        ...
        int[][] indsCharListsMultiTree = phenotypicForSurrogate.muchBetterPhenotypicPopulation(this, phenoCharacterisation[generation]);
...
//            double fitness = fitnessList.stream().mapToDouble(d -> d).average().orElse(0.0);    // choose average
        ...
        if (generation >= switchGen) {
            ...
            Map<Integer, int[][]> instanceToPhenoMap = new HashMap<>();
            ...
            for (int instance = 0; instance < surrogatePCEveryGen.size() - 1; instance++) {
                int[][] indsCharListsMultiTreeCurrentGen = phenotypicForSurrogate.muchBetterPhenotypicPopulation(this, phenoCharacterisation[instance]);
                instanceToPhenoMap.put(instance, indsCharListsMultiTreeCurrentGen);
                int samePCNum = 0;
                for (Map.Entry<List<Integer>, List<Integer>> pcToIndex : pcToIndexMap.entrySet()) {
                    ...
                    if (dMin == 0) {
                        samePCNum++;
                        indToMatchingInstances.computeIfAbsent(pcToIndex.getValue().get(0), k -> new ArrayList<>()).add(instance);
                        ...
                        for (int indsIndex : pcToIndex.getValue()) {
                            ((GPIndividual) population.subpops[0].individuals[indsIndex]).caseFitness.add(Double.MAX_VALUE);
                        }
                    }
                }
...
//            firstSelectedCasesIndex.add(generation);
            ...
            if(firstSelectedCasesIndex.size()> 15){
            ...
            for (int instance = 0; instance < surrogatePCEveryGen.size() - 1; instance++) {
                ...
                if (finalSelectedCasesIndex.contains(instance)) {
                    ...
                    int[][] indsCharListsMultiTreeCurrentGen = instanceToPhenoMap.get(instance);
                    ...
                    for (Map.Entry<List<Integer>, List<Integer>> pcToIndex : pcToIndexMap.entrySet()) {
                        ...
                        if (dMin > pcDistance) { //use real evaluation to replace the estimation
                            GPIndividual indi = (GPIndividual) population.subpops[0].individuals[pcToIndex.getValue().get(0)];
                            ...
                            simulation.reset();
                            for (int ind : pcToIndex.getValue()) {
            ...
            System.out.println("The number of evaluations is " + (realEvaluationNum+pcToFitnessMap.size()));
