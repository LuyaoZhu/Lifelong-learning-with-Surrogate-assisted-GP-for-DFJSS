 ...
 */
public class EpsilonLexicaseSelectionV3 extends TournamentSelection {
...
//        if (((GPRuleEvolutionStateSavedSurrogateCasesV3) state).finalSelectedCasesIndex.size() < 10) {
        if (state.generation < ((GPRuleEvolutionStateSavedSurrogateCasesV3) state).switchGen) {
...
//        System.out.println("TourIndividuals' size has not arrive at the set size in advance.");
    ...
    public List<Integer> getRandomSequenceInstance(EvolutionState state, int thread) {
        ...
        for (int i = 0; i < ((GPRuleEvolutionStateSavedSurrogateCasesV3) state).finalSelectedCasesIndex.size(); i++) {
            Instance.add(((GPRuleEvolutionStateSavedSurrogateCasesV3) state).finalSelectedCasesIndex.get(i));
        }
