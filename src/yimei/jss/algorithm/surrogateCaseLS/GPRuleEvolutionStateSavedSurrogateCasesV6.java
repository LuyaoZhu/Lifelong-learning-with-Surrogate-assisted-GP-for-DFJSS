...
public class GPRuleEvolutionStateSavedSurrogateCasesV6 extends GPRuleEvolutionState {
    ...
    ArrayList<double[]> surrogateFitnessEveryGenNoMap = new ArrayList<>();
    ArrayList<int[][][]> surrogatePCEveryGenNoMap = new ArrayList<>();
    ...
    ArrayList<Integer> extraEvaluateNum = new ArrayList<>();
    ...
    public static ArrayList<Double> individualBeforeFitness = new ArrayList<>();
    public static ArrayList<Double> individualAfterFitness = new ArrayList<>();
    public static ArrayList<Integer> sameIndividualsIndex = new ArrayList<>();
    ...
    public void setup(EvolutionState state, Parameter base) {
        ...
        phenoCharacterisation[0] = SequencingPhenoCharacterisation.currentGenPhenoCharacterisation(this, problem);
        phenoCharacterisation[1] = RoutingPhenoCharacterisation.currentGenPhenoCharacterisation(this, problem);
    ...
    public int evolve() {
        ...
        int[][][] indsCharListsMultiTree = phenotypicForSurrogate.muchBetterPhenotypicPopulationRanks(this, phenoCharacterisation);
        for (int ind = 0; ind < this.population.subpops[0].individuals.length; ind++) {
            ...
            individual.NPC = indsCharListsMultiTree[ind];
        }
...
//        entropyDiversity.add(diversityValue);
        ...
        //now we know the fitness of same individuals among different instances, build linear regression model to increase the number of sample
        ArrayList<int[][]> mappedPC = new ArrayList<>();
        ArrayList<Double> mappedFitness = new ArrayList<>();
        if(generation > 0){
            mappedSurrogate(indsCharListsMultiTree,mappedPC,mappedFitness);
        }

        //-----------------------begin--------------------------
        //after evaluate all individuals, we record the information to build surrogate in this generation
        double[] fitnessesForModel = new double[population.subpops[0].individuals.length];
        ...
        double[] tempfitnessesForModel = fitnessesForModel;
        int[][][] tempindsCharListsMultiTree = indsCharListsMultiTree;

        surrogateFitnessEveryGenNoMap.add(tempfitnessesForModel.clone());
        surrogatePCEveryGenNoMap.add(tempindsCharListsMultiTree.clone());

        //combine mapped with original
        tempindsCharListsMultiTree = Stream.concat(Arrays.stream(tempindsCharListsMultiTree), mappedPC.stream()).toArray(int[][][]::new);
        tempfitnessesForModel = DoubleStream.concat(Arrays.stream(tempfitnessesForModel), mappedFitness.stream().mapToDouble(Double::doubleValue)).toArray();
        ...
        if(generation == switchGen-1){
            ...
            for(int instance = 0; instance <= generation; instance++){
                double[] fitnessInOneCase = new double[topIndividuals.size()];
                for (int i = 0; i < topIndividuals.size(); i++) {
                    GPIndividual indi = (GPIndividual) topIndividuals.get(i);
                ...
                fitnessInDifferentCases.add(fitnessInOneCase);
            }
        ...
        if (generation >= switchGen) {
            ...
            //evaluate the individuals in the current generation
                double[] fitnessInOneCase = new double[topIndividuals.size()];
                for (int i = 0; i < topIndividuals.size(); i++) {
                    GPIndividual indi = (GPIndividual) topIndividuals.get(i);
                    GPRule sequencingRule = new GPRule(RuleType.SEQUENCING, indi.trees[0]);
            ...
            for (int ind = 0; ind < this.population.subpops[0].individuals.length; ind++) {
                GPIndividual individual = (GPIndividual) population.subpops[0].individuals[ind];
                individual.caseFitness = new ArrayList<>(Collections.nCopies(generation+1, 0.0)); // to save the estimated fitness in all cases
                individual.realCaseFitness = new ArrayList<>();
...
//            selectCases();
            ...
            double[][] spearmanMatrix = calcSpearmanMatrix(fitnessInDifferentCases);
            finalSelectedCasesIndex = selectCases(spearmanMatrix, 25, correlationThreshold,5);
        ...
        caseNum.add(finalSelectedCasesIndex.size());
...
// SHOULD WE QUIT?
        ...
        if (generation == numGenerations - 1) {
            ...
            writeExtraEvaluationNumToFile(extraEvaluateNum);
...
//            writeFirstSelectedCasesNumToFile(firstSelectedCasesNum);
        ...
        samePCNumInCases.clear();
        averagePCDistance.clear();
...
// INCREMENT GENERATION AND CHECKPOINT
    ...
    private void mappedSurrogate(int[][][] indsCharListsMultiTree,ArrayList<int[][]> mappedPC,ArrayList<Double> mappedFitness) {
        for(int i=0; i<sameIndividualsIndex.size(); i++) {
            Individual individual = population.subpops[0].individuals[sameIndividualsIndex.get(i)];
            individualAfterFitness.add(individual.fitness.fitness());
        }

        double[] coefficient = LinearRegression.fit(individualBeforeFitness, individualAfterFitness);
        for (int sample = 0; sample < surrogateFitnessEveryGenNoMap.get(generation-1).length; sample++) {
            double fitnessBefore = surrogateFitnessEveryGenNoMap.get(generation-1)[sample];
            double predictFitness = LinearRegression.predict(coefficient[0],coefficient[1],fitnessBefore);
            mappedFitness.add(predictFitness);
            mappedPC.add(surrogatePCEveryGenNoMap.get(generation-1)[sample]);
        }

        individualBeforeFitness.clear();
        individualAfterFitness.clear();
        sameIndividualsIndex.clear();

        //remove the sample with same behaviour
        for (int i=0; i< mappedPC.size(); i++) {
            int[][] pc = mappedPC.get(i);
            for (int[][] pcGen : indsCharListsMultiTree) {
                if(PhenoCharacterisation.spearmanDistance(pc, pcGen) == 0){
                    mappedPC.remove(i);
                    mappedFitness.remove(i);
                    i--;
                    break;
                }
            }
    ...
    private void assignFitness() {
        ...
        for (int instance = 0; instance < surrogatePCEveryGen.size() - 1; instance++) {
            ...
            if (finalSelectedCasesIndex.contains(instance)) {

                for (int ind=0; ind < population.subpops[0].individuals.length; ind++) {
                    int[][] estimatedPC = ((GPIndividual)population.subpops[0].individuals[ind]).NPC;
                    double dMin = Double.MAX_VALUE;
                    int index = 0;
                    for (int pc = 0; pc < surrogatePCEveryGen.get(instance).length; pc++) {
                        int[][] pcModel = surrogatePCEveryGen.get(instance)[pc];
                        double d = PhenoCharacterisation.spearmanDistanceV1(estimatedPC, pcModel);
                        if (d == 0) {
                            dMin = d;
                            index = pc;
                            break;
                    ...
                    ((GPIndividual) population.subpops[0].individuals[ind]).caseFitness.set(instance, surrogateFitnessEveryGen.get(instance)[index]);
                }
            }
        }
        ...
        for (int ind = 0; ind < this.population.subpops[0].individuals.length; ind++) {
            GPIndividual individual = (GPIndividual) population.subpops[0].individuals[ind];
            individual.caseFitness.set(generation, individual.fitness.fitness());
//            individual.realCaseFitness.add(individual.fitness.fitness());
        }
    ...
    private void selectCases() {
        ...
        //1. determine the cases
        for (int instance = 0; instance < surrogatePCEveryGen.size() - 1; instance++) {
            double NPCDistance = 0;
            for (int ind=0; ind < population.subpops[0].individuals.length; ind++) {
                int[][] estimatedPC = ((GPIndividual)population.subpops[0].individuals[ind]).NPC;
                double dMin = Double.MAX_VALUE;
                int index = 0;
                ...
                NPCDistance += dMin;
                ((GPIndividual) population.subpops[0].individuals[ind]).caseFitness.add(surrogateFitnessEveryGen.get(instance)[index]);

            }
            averagePCDistance.add(NPCDistance/population.subpops[0].individuals.length);
        }

        long endTime = System.currentTimeMillis();
        System.out.println("The time used is " +  (endTime - startTime)/1000 + "s");

        //2.use knee-point to select cases
        KneePointFinder.KneeResult result = KneePointFinder.findKneePoint(averagePCDistance, 0);

        // 创建索引列表
        List<Integer> indices = new ArrayList<>();
        for (int i = 0; i < averagePCDistance.size(); i++) {
            indices.add(i);
        }
        // 按距离值升序排序
        indices.sort(Comparator.comparingDouble(averagePCDistance::get));

        // 取前 25 个（如果不足 25 个则取全部）
        int limit = Math.min(25, indices.size());
        for (int i = 0; i < limit; i++) {
            int idx = indices.get(i);
            if (averagePCDistance.get(idx) < result.kneeValue) {
                firstSelectedCasesIndex.add(idx);
                System.out.print(idx + ", ");
            }
        }

        // find all the point that
/*        System.out.print("The index of selected cases are ");
        for (int i = 0; i < averagePCDistance.size(); i++) {
            if (averagePCDistance.get(i) < result.kneeValue) {
                firstSelectedCasesIndex.add(i);
                System.out.print(i + ", ");
            }
        }*/

        finalSelectedCasesIndex = firstSelectedCasesIndex;
...
//            terminalsFrom[subPopNum] = subPop1TerminalSet;
    ...
    public void writeExtraEvaluationNumToFile(ArrayList<Integer> extraEvaluationNum) {
        // fzhang 2019.5.21 save the number of cleared individuals
        File weightFile = new File(out_dir + "/job." + jobSeed + ".extraEvaluationNum.csv"); // jobSeed = 0
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter(weightFile));

            // Dynamically create the header based on the size of entropyDiversity elements
            StringBuilder header = new StringBuilder("Gen");
            if (!extraEvaluationNum.isEmpty()) {
                for (int j = 0; j < 1; j++) {
                    header.append(",extraEvaluationNum").append(j);
                }
            }
            writer.write(header.toString());
            writer.newLine();
            ...
            // Write the data
            for (int i = 0; i < extraEvaluationNum.size(); i++) {
                StringBuilder line = new StringBuilder(i + switchGen + "");
                line.append(", ").append(extraEvaluationNum.get(i));
                writer.write(line.toString());
                writer.newLine();
            }

            extraEvaluationNum.clear();
            writer.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
...
// 生成排名数组
    ...
    public static double[][] calcSpearmanMatrix(ArrayList<double[]> dataList) {
        int n = dataList.size();
        double[][] result = new double[n][n];
        SpearmansCorrelation sc = new SpearmansCorrelation();

        for (int i = 0; i < n; i++) {
            result[i][i] = 1.0; // 自己和自己相关性为1
            for (int j = i + 1; j < n; j++) {
                double corr = sc.correlation(dataList.get(i), dataList.get(j));
                result[i][j] = corr;
                result[j][i] = corr; // 对称矩阵
            }
        }
        return result;
    }

    public static ArrayList<Integer> selectCases(double[][] spearmanMatrix, int maxCount, double corrThreshold, int skipCount) {
        ...
        while (selected.size() < maxCount) {
            ...
            for (int candidate = 0; candidate < n; candidate++) {
                if (selected.contains(candidate) || candidate < skipCount) continue;
