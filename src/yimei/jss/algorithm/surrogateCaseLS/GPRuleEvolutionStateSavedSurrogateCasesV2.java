...
/**
 * select top 60 inds base on the fitness in the current generation (case)
 * and then truly evaluate their performance in the nearest 20 cases
 * then use LS
 * @author yimei
 */

public class GPRuleEvolutionStateSavedSurrogateCasesV2 extends GPRuleEvolutionState {
    ...
    public ArrayList<Integer> topIndsIndex = new ArrayList<>();
...
//        System.out.println(Arrays.stream(fitnessesForModelCopy).average().getAsDouble());
        ...
        if (generation >= switchGen ) {
            ...
            for (int gen = generation; gen >= generation - 20; gen--) {
                finalSelectedCasesIndex.add(gen);
            }

            //----------------------------select top 10% inds

            double[] topInds = getTop100Min(fitnessesForModelCopy);

            for (int ind = 0; ind < this.population.subpops[0].individuals.length; ind++) {
                GPIndividual individual = (GPIndividual) population.subpops[0].individuals[ind];
                individual.caseFitness = new ArrayList<>(); // to save the estimated fitness in all cases
                if(individual.fitness.fitness() <= topInds[topInds.length-1]) {
                    topIndsIndex.add(ind);
                }
            }

            for (int instance = 0; instance < surrogatePCEveryGen.size() - 1; instance++) {

                for (int indIndex: topIndsIndex) {

                    GPIndividual indi = (GPIndividual) population.subpops[0].individuals[indIndex];

                    if (finalSelectedCasesIndex.contains(instance)) {  //use real evaluation
                        ...
                        indi.caseFitness.add(ObjValue);

                        simulation.reset();

                    } else { //whatever, give a random value
                        indi.caseFitness.add(Double.MAX_VALUE);
                    }
                }
            }
...
// 生成排名数组
    ...
    public double[] getTop100Min(double[] arr) {
        // 创建一个副本，避免修改原数组
        ...
        return Arrays.copyOf(copy, (int) Math.min(this.population.subpops[0].individuals.length * 0.05, copy.length));
    }
