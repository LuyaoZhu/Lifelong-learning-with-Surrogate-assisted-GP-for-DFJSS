...
public class AllIndexCrossoverPipelineLS extends AllIndexCrossoverPipeline {
...

    {
        ...
        for (int q = start; q < n + start; /* no increment */)  // keep on going until we're filled up
        {
            ...
            if (tree1 == TREE_UNFIXED && tree2 == TREE_UNFIXED && (parents[0].trees.length == parents[1].trees.length)) {
                ...
                inds[q] = j1;
/*                if(parents[0].fromTS && parents[1].fromTS){
                    ((GPRuleEvolutionStateSavedSurrogateCases)state).offspringTS.add(q);
                }else if( (!parents[0].fromTS) && (!parents[1].fromTS)){
                    ((GPRuleEvolutionStateSavedSurrogateCases)state).offspringLS.add(q);
                }else {
                    ((GPRuleEvolutionStateSavedSurrogateCases)state).offspringMixed.add(q);
                }*/
                q++;
