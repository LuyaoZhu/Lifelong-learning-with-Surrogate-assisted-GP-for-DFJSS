...
import yimei.jss.niching.surrogateClearing;
...
public class EnhanceCorrelationGPRuleEvolutionStateTest extends GPRuleEvolutionState {
    ...
    public double[][][] beforeAfterChangedAllFitnessGen = new double[2][250][2];  //record all individuals' fitness before changing

    public double[][][] beforeAfterChangedFitnessGen;   //subpop ,individuals, before and after fitness

    public boolean changeElites = true;

    public int[][][][] cases = new int[2][4][4][2];  //record baseline's cases, the fist matrix is +,-,*,/,  the second matrix is P&P, P&N, N&P, N&N
...
//                    if (individuals[indIndex].fitness.fitness() != Double.POSITIVE_INFINITY & individuals[indIndex].fitness.fitness() != Double.MAX_VALUE)
                        beforeAfterChangedAllFitnessGen[subIndex][indIndex][0] = individuals[indIndex].fitness.fitness();
                    }
                }
            }
        }
...
//                int nonterminals = tree.child.numNodes(GPNode.NODESEARCH_NONTERMINALS);
                int nonterminals = tree.child.numNodes(GPNode.NODESEARCH_ALL);
...
//                                GPNode node = treeClone.child.nodeInPosition(numSubtree, GPNode.NODESEARCH_NONTERMINALS);
                                GPNode node = treeClone.child.nodeInPosition(numSubtree, GPNode.NODESEARCH_ALL);
                        ...
                        //record cases
                        recordCases(((GPIndividual) selIndis[sub][ind]).trees[0].child, score, sub);
...
//        System.out.println("------After Changing the functions------");
        ...
        for (int subIndex = 0; subIndex < population.subpops.length; subIndex++) {
            ...
            for (int indIndex = 0; indIndex < population.subpops[subIndex].individuals.length; indIndex++) {
                //System.out.println(individuals[indIndex].fitness.fitness());
//                    if (individuals[indIndex].fitness.fitness() != Double.POSITIVE_INFINITY & individuals[indIndex].fitness.fitness() != Double.MAX_VALUE)
                beforeAfterChangedAllFitnessGen[subIndex][indIndex][1] = individuals[indIndex].fitness.fitness();
            }
        }
...
//        if (generation >= 0) {
            ...
            for (int subIndex = 0; subIndex < population.subpops.length; subIndex++) {
                ...
                for (int ind = 0; ind < changedIndIndex.get(subIndex).size(); ind++) {
                    beforeAfterChangedFitnessGen[subIndex][ind][0] = beforeAfterChangedAllFitnessGen[subIndex][changedIndIndex.get(subIndex).get(ind)][0];
            ...
            double [][][] caseRatio = new double[2][4][4];
            for (int sub=0; sub<cases.length; sub++){
                for (int f=0; f<cases[sub].length; f++){
                    for (int c=0; c<cases[sub][f].length; c++)
                        caseRatio[sub][f][c] = ((double) cases[sub][f][c][0])/(cases[sub][f][c][0]+cases[sub][f][c][1]);
                }
            }
            System.out.println("         +        " + "            -         " +  "            *        " +  "            /           ");
            System.out.println("P&P" + caseRatio[0][0][0] + " , " + caseRatio[0][1][0] + " , " +caseRatio[0][2][0] + " , " + caseRatio[0][3][0]);
            System.out.println("P&N" + caseRatio[0][0][1] + " , " + caseRatio[0][1][1] + " , " +caseRatio[0][2][1] + " , " + caseRatio[0][3][1]);
            System.out.println("N&P" + caseRatio[0][0][2] + " , " + caseRatio[0][1][2] + " , " +caseRatio[0][2][2] + " , " + caseRatio[0][3][2]);
            System.out.println("N&N" + caseRatio[0][0][3] + " , " + caseRatio[0][1][3] + " , " +caseRatio[0][2][3] + " , " + caseRatio[0][3][3]);
...
//        seletedIndsKneePointEveryGen.add(seletedIndsKneePoint);
    ...
    private void recordCases(GPNode child, double[] score, int sub) {
        ...
        for (int i = 0; i < score.length; i++) {
            GPNode parentNode = child.nodeInPosition(i, GPNode.NODESEARCH_ALL);

            if (parentNode.numNodes(GPNode.NODESEARCH_ALL) == 1) {
                continue;
            }

            int leftChildIndex = i + 1;
            int rightChildIndex = parentNode.children[0].numNodes(GPNode.NODESEARCH_ALL) + 1;

            if (parentNode.toStringForHumans().equals(nodes[1].toStringForHumans())) {      //parent node is +
                judgeCorrelation(score, leftChildIndex, rightChildIndex, cases[sub][0]);
            } else if (parentNode.toStringForHumans().equals(nodes[2].toStringForHumans())) {      //parent node is +
                judgeCorrelation(score, leftChildIndex, rightChildIndex, cases[sub][1]);
            } else if (parentNode.toStringForHumans().equals(nodes[3].toStringForHumans())) {      //parent node is +
                judgeCorrelation(score, leftChildIndex, rightChildIndex, cases[sub][2]);
            } else if (parentNode.toStringForHumans().equals(nodes[4].toStringForHumans())) {      //parent node is +
                judgeCorrelation(score, leftChildIndex, rightChildIndex, cases[sub][3]);
            }
        }
    }

    private void judgeCorrelation(double[] score, int leftChildIndex, int rightChildIndex, int[][] caseIndex) {
        if (score[leftChildIndex] >= 0 && score[rightChildIndex] >= 0) {
            if (score[leftChildIndex - 1] >= 0)
                caseIndex[0][0]++;
            else
                caseIndex[0][1]++;
        } else if (score[leftChildIndex] >= 0 && score[rightChildIndex] <= 0) {
            if (score[leftChildIndex - 1] >= 0)
                caseIndex[1][0]++;
            else
                caseIndex[1][1]++;
