...
public class SimpleStatisticsSaveFitnessCCGP extends Statistics implements SteadyStateStatisticsForm //, ec.eval.ProvidesBestSoFar
{
    ...
    List<Double> aveOrdRulesizeSubPop2 = new ArrayList<>();
...
*/
    ...
    public void preEvaluationStatistics(final EvolutionState state) {
        ...
        if (state.population.subpops.length == 3) {
            for (int ind = 0; ind < state.population.subpops[2].individuals.length; ind++) {
                OrdSizePop2 += state.population.subpops[2].individuals[ind].size();
            }
            aveOrdSizePop2 = OrdSizePop2 / state.population.subpops[2].individuals.length;
        }
        ...
        aveOrdRulesizeSubPop2.add(aveOrdSizePop2);
...
//		    File rulesizeFile = new File("job." + jobSeed + ".aveGenRulesize.csv"); // jobSeed = 0
            ...
            try {
                ...
                writer.write("Gen,aveSeqRuleSize,aveRouRuleSize,aveOrdRuleSize,aveSize");
                writer.newLine();
                for (int gen = 0; gen < aveSeqRulesizeSubPop0.size(); gen++) {
                    writer.write(gen + "," + aveSeqRulesizeSubPop0.get(gen) + "," + aveRouRulesizeSubPop1.get(gen) + "," + +aveOrdRulesizeSubPop2.get(gen) + ","
                            + (aveSeqRulesizeSubPop0.get(gen) + aveRouRulesizeSubPop1.get(gen) + aveOrdRulesizeSubPop2.get(gen)) / state.population.subpops.length);
                    writer.newLine();
                }
                writer.close();
            } catch (IOException e) {
