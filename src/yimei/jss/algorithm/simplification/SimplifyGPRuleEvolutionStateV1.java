 ...
 * 1.replace the fullTree with the most relevant subtree;(set a probability to determine if simplifying this individual)
 * 2.replace the unimportant subtrees with important 2-layer subtrees (relevant the elites) without 2
 */
public class SimplifyGPRuleEvolutionStateV1 extends GPRuleEvolutionState {
...
//        ArrayList<GPTree> routingFeature = new ArrayList<>();
        for (int sub = 0; sub < state.population.subpops.length; sub++) {
            for (int ind = 0; ind < state.population.subpops[sub].individuals.length; ind++) {
                //set a probability to determine if simplifying this individual
...
//                                }
                            ...
                            for (int numScore = 1; numScore < score.length; numScore++) {
                                if (score[numScore] > maxCorrelation) {
                                    maxCorrelation = score[numScore];
                                }
                            }
                            System.out.println(maxCorrelation);
                            //也许相关性最高的有多个子树
                            ...
                            int randomNumber = this.random[0].nextInt(selectedIndexes.size());
                            int index = selectedIndexes.get(randomNumber);
...
/*                            //从中选出最小节点的子树,确定其索引
                           ...
                           /* //从中选出最大节点的子树,确定其索引
                            ...
                            for (int a = 0; a < selectedIndexes.size(); a++) {
                                ...
                                GPTree nodeToTree = PopulationUtils.GPNodetoGPTree(node);
                                int sumNodes = nodeToTree.child.numNodes(GPNode.NODESEARCH_TERMINALS) + nodeToTree.child.numNodes(GPNode.NODESEARCH_NONTERMINALS);
                                if (sumNodes > maxNumNodes) {
                                    maxNumNodes = sumNodes;
                                    index = selectedIndexes.get(a);
                                }
                            }*/
