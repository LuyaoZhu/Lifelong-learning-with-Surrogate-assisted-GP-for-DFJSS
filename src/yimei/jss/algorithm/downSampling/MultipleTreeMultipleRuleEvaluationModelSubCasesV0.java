...
public class MultipleTreeMultipleRuleEvaluationModelSubCasesV0 extends MultipleTreeMultipleRuleEvaluationModel {
...
/*        if (rules.size() != 2) {
        ...
        if(state.generation >= ((GPRuleEvolutionStateDownSampling)state).switchGen) {
            for(Simulation simulation : ((GPRuleEvolutionStateDownSampling)state).finalSelectedSimulations) {
                simulations.add( ((DynamicSimulation)simulation).deepCloneAllSame());
            }
        } else {
...
//                fitnesses[i] += normObjValue;  //the value of fitness is the normalization of the objective value
        ...
        for (int i = 0; i < fitnesses.length; i++) {
            if (Double.isNaN(fitnesses[i]) || Double.isInfinite(fitnesses[i])) {
