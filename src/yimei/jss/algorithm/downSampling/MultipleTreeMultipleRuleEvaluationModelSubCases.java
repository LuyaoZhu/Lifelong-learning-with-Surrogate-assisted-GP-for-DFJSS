...
public class MultipleTreeMultipleRuleEvaluationModelSubCases extends MultipleTreeMultipleRuleEvaluationModel {
...
//                        / schedulingSet.getObjectiveLowerBound(i, col);
                ...
                double ObjValue;
                double[] caseObjective = new double[((GPRuleEvolutionStateDownSampling)state).caseInOneInstance];
                if(simulation.getSystemState().getClockTime() == Double.MAX_VALUE) { //means this is a bad run
                    if(objectives.get(0).getName().endsWith("profit")) {
                        ...
                        Arrays.fill(caseObjective, -Double.MAX_VALUE);
                    }
                    else {
                        ObjValue = Double.MAX_VALUE;
                        Arrays.fill(caseObjective, Double.MAX_VALUE);
                    ...
                    caseObjective = simulation.objectiveValueMultiCase(objectives.get(i),((GPRuleEvolutionStateDownSampling)state).caseInOneInstance);
