...
public class SimpleStatisticsSaveRulesizeGen extends SimpleStatistics {
    ...
    static List<Long> aveSeqRulesizeTree0 = new ArrayList<>();
    static List<Long> aveRouRulesizeTree1 = new ArrayList<>();
    static List<Long> aveOrdRulesizeTree2 = new ArrayList<>();
    ...
    public void preEvaluationStatistics(final EvolutionState state) {
        ...
        for (int ind = 0; ind < state.population.subpops[0].individuals.length; ind++) {
            ...
            if (indi.trees.length == 3)
                OrdSizeTree2 += indi.trees[2].child.numNodes(GPNode.NODESEARCH_ALL);
        }
        ...
        aveOrdSizeTree2 = OrdSizeTree2 / state.population.subpops[0].individuals.length;

        aveSeqRulesizeTree0.add(aveSeqSizeTree0);
        aveRouRulesizeTree1.add(aveRouSizeTree1);
        aveOrdRulesizeTree2.add(aveOrdSizeTree2);

    }

    public static void writeAveGenRulesizeToFile(int treeNum) {
        // fzhang 2019.5.21 save the number of cleared individuals
        File rulesizeFile = new File(out_dir + "/job." + jobSeed + ".aveGenRulesize.csv");

        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter(rulesizeFile));
            writer.write("Gen,aveSeqRuleSize,aveRouRuleSize,aveOrdRuleSize,avePairSize");
            writer.newLine();
            for (int gen = 0; gen < aveSeqRulesizeTree0.size(); gen++) {
                writer.write(gen + "," + aveSeqRulesizeTree0.get(gen) + "," + aveRouRulesizeTree1.get(gen) + "," + aveOrdRulesizeTree2.get(gen) + "," +
                        (aveSeqRulesizeTree0.get(gen) + aveRouRulesizeTree1.get(gen) + aveOrdRulesizeTree2.get(gen)) / treeNum);
                writer.newLine();
            }
            writer.close();
        } catch (IOException e) {
