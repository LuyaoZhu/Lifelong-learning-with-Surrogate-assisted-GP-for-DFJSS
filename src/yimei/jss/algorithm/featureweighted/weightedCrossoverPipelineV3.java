...
import static yimei.jss.algorithm.featureweighted.weightedKozaNodeSelectorV9.indexForParent2;

public class weightedCrossoverPipelineV3 extends CrossoverPipeline {
    static int toolarge = 0;
...

    {
        ...
        for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
        {
           ...
           /* Produces <i>n</i> individuals from the given subpopulation
            ...
            {
                // pick a bad node in individual 1
                p1 = nodeselect1.pickNode(state,subpopulation,thread,parents[0],parents[0].trees[t1], true); //the small (unimportant), the better.
                //pick a good node in individual 1
                //if (indexForParent2 != -1) {
                p3 = parents[0].trees[t1].child.nodeInPosition(indexForParent2, GPNode.NODESEARCH_NONTERMINALS);//the small (unimportant), the better.
                indexForParent2 = -1;
               // }

                // pick a good node in individual 2
                p2 = nodeselect2.pickNode(state, subpopulation, thread, parents[1], parents[1].trees[t2], false); // the larger (important), the better.
                // pick a bad node in individual 2
               // if (indexForParent2 != -1) {
                p4 = parents[1].trees[t2].child.nodeInPosition(indexForParent2, GPNode.NODESEARCH_NONTERMINALS); // the larger (important), the better.
                indexForParent2 = -1;
               // }
                ...
                if(p3 == null || p4 == null){
                    tossSecondParent = true;
                }

                if (n-(q-start)<2 || tossSecondParent) res2 = true;
                else {
                    //res2 = verifyPoints(initializer,p1,p2);  // p1 can fill p2's spot -- order is important!
                    res2 = verifyPoints(initializer, p3, p4);
                }


                //fzhang 2019.11.11 if the subtree is not suitable for swap, record the times that happen.
