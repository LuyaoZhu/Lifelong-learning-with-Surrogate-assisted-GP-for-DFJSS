 ...
 */
public class FreGPRuleEvolutionStateV0 extends GPRuleEvolutionState {
...
//        phenoCharacterisation = new PhenoCharacterisation[numGenerations][2];
...
//        phenoCharacterisation[generation][0] = SequencingPhenoCharacterisation.currentGenPhenoCharacterisation(this, problem);
//        phenoCharacterisation[generation][1] = RoutingPhenoCharacterisation.currentGenPhenoCharacterisation(this, problem);
        ...
        System.out.println(((GPIndividual)population.subpops[0].individuals[0]).trees[0].child.makeGraphvizTree());
        //luyao 8/7/2022 用hashmap记录下每个重复PC的个数
        /*HashMap<double[], Integer> duplicatesSeq = new HashMap<>();
        for (int i = 0; i < (indsCharListsMultiTree[0]).length; i++) {
            int judge = 0;
            for (double[] k : duplicatesSeq.keySet()) {
                if (Arrays.equals(k, indsCharListsMultiTree[0][i])) {
                    judge = 1;
                    int tempValue = duplicatesSeq.get(k);
                    tempValue++;
                    duplicatesSeq.replace(k, tempValue);
                    break;
                }
            }
            if (judge == 0)
                duplicatesSeq.put(indsCharListsMultiTree[0][i], 1);
        }
        ArrayList<Map.Entry<double[], Integer>> listPcClusterSeq = new ArrayList<>(duplicatesSeq.entrySet());
        //通过list对hashmap排序
        Collections.sort(listPcClusterSeq, new Comparator<ConcurrentHashMap.Entry<double[], Integer>>() {
            @Override
            public int compare(ConcurrentHashMap.Entry<double[], Integer> o1, ConcurrentHashMap.Entry<double[], Integer> o2) {
                //按照value值，从大到小排序
        ...
        HashMap<double[], Integer> duplicatesRou = new HashMap<>();
        for (int i = 0; i < (indsCharListsMultiTree[1]).length; i++) {
            int judge = 0;
            for (double[] k : duplicatesRou.keySet()) {
                if (Arrays.equals(k, indsCharListsMultiTree[1][i])) {
                    judge = 1;
                    int tempValue = duplicatesRou.get(k);
                    tempValue++;
                    duplicatesRou.replace(k, tempValue);
                    break;
                }
            }
            if (judge == 0)
                duplicatesRou.put(indsCharListsMultiTree[1][i], 1);
        }
        ArrayList<Map.Entry<double[], Integer>> listPcClusterRou = new ArrayList<>(duplicatesRou.entrySet());
        //通过list对hashmap排序
        Collections.sort(listPcClusterRou, new Comparator<ConcurrentHashMap.Entry<double[], Integer>>() {
            @Override
            public int compare(ConcurrentHashMap.Entry<double[], Integer> o1, ConcurrentHashMap.Entry<double[], Integer> o2) {
                //按照value值，从大到小排序
                return o2.getValue() - o1.getValue();
...
/*        if(this.generation > 0){
            int sameSeqNum = 0;
            for(int a=0; a< seqFeatureArchive.get(this.generation).size(); a++){
                GPTree base = (GPTree) seqFeatureArchive.get(this.generation).get(a).clone();
                for(int b=0; b< seqFeatureArchive.get(this.generation-1).size(); b++){
                    GPTree count = (GPTree) seqFeatureArchive.get(this.generation-1).get(b).clone();
                    if (base.child.makeLispTree().equals(count.child.makeLispTree())) {
                        sameSeqNum++;
                    }
                }
            }
            System.out.println("the number of seqFeatures in generation"+ (this.generation-1)+ "is" + (seqFeatureArchive.get(this.generation-1).size()));
            System.out.println("the number of same seqFeatures between generation"+ (this.generation-1) + "and" + (this.generation)+ "is" + sameSeqNum);
            System.out.println("the number of seqFeatures in generation"+ (this.generation)+ "is" + (seqFeatureArchive.get(this.generation).size()));

            int sameRouNum = 0;
            for(int a=0; a< rouFeatureArchive.get(this.generation).size(); a++){
                GPTree base = (GPTree) rouFeatureArchive.get(this.generation).get(a).clone();
                for(int b=0; b< rouFeatureArchive.get(this.generation-1).size(); b++){
                    GPTree count = (GPTree) rouFeatureArchive.get(this.generation-1).get(b).clone();
                    if (base.child.makeLispTree().equals(count.child.makeLispTree())) {
                        sameRouNum++;
                    }
                }
            ...
            System.out.println("the number of rouFeatures in generation"+ (this.generation-1)+ "is" + (rouFeatureArchive.get(this.generation-1).size()));
            System.out.println("the number of same rouFeatures between generation"+ (this.generation-1) + "and" + (this.generation)+ "is" + sameRouNum);
            System.out.println("the number of rouFeatures in generation"+ (this.generation)+ "is" + (rouFeatureArchive.get(this.generation).size()));
...
/*        for(int a=0; a<FeatureConstructionCrossoverPipelineV0.featureNumInOneTree.length; a++){
            FeatureConstructionCrossoverPipelineV0.featureNumInOneTree[a] = 0;
        }
        for(int a=0; a<FeatureConstructionCrossoverPipelineV0.caseNum.length; a++){
            FeatureConstructionCrossoverPipelineV0.caseNum[a] = 0;
        }

        for(int a=0; a<FeatureConstructionCrossoverPipelineV0.case1.length; a++){
            FeatureConstructionCrossoverPipelineV0.case1[a] = 0;
        }*/

/*        ArrayList<GPTree> Subpop0protectedSubtrees = new ArrayList<>();
        ArrayList<GPTree> Subpop1protectedSubtrees = new ArrayList<>();

        for (int pop = 0; pop < this.population.subpops.length; pop++) {
            for (int ind = 0; ind < this.population.subpops[pop].individuals.length; ind++) {
                GPTree tree = ((GPIndividual) this.population.subpops[pop].individuals[ind]).trees[0];
                if (tree.child.numNodes(GPNode.NODESEARCH_NONTERMINALS) > 1) {
                    ...
                    for (int numSubtree = 0; numSubtree < nonterminals; numSubtree++) {
                        ...
                        if(pop ==0) {
                        for (int f = 0; f < seqFeatureArchive.get(this.generation).size(); f++) {
                            GPTree countTree = (GPTree)seqFeatureArchive.get(this.generation).get(f).clone();
                            if (baseTree.child.makeLispTree().equals(countTree.child.makeLispTree())) {
                                Subpop0protectedSubtrees.add(countTree);
                            }
                        }
                    }else {
                            for (int f = 0; f < rouFeatureArchive.get(this.generation).size(); f++) {
                                GPTree countTree = (GPTree) rouFeatureArchive.get(this.generation).get(f).clone();
                                if (baseTree.child.makeLispTree().equals(countTree.child.makeLispTree())) {
                                    Subpop1protectedSubtrees.add(countTree);
                                }
                            }
                        }
                    }
        ...
        System.out.println("The number of features in the popolation 0 is " + Subpop0protectedSubtrees.size());
        System.out.println("The number of features in the popolation 1 is " + Subpop1protectedSubtrees.size());
        Subpop0protectedSubtrees.clear();
        Subpop1protectedSubtrees.clear();*/
...
//        System.out.println(" the number of zeroInds and nonZeroInds are " + zeroNum + " and " + nonZeroNum);
//
//        zeroNum = 0;
//        nonZeroNum = 0;
...
/*        for(int a=0; a<FeatureConstructionCrossoverPipelineV0.caseNum.length; a++){
            System.out.println("case " + (a+1) + " have " + FeatureConstructionCrossoverPipelineV0.caseNum[a]);
        }
        for(int a=0; a<FeatureConstructionCrossoverPipelineV0.case1.length; a++){
            System.out.println("case 1's tree  " + (a+1) + " have " + FeatureConstructionCrossoverPipelineV0.case1[a]);
        }

        for(int a=0; a<FeatureConstructionCrossoverPipelineV0.featureNumInOneTree.length; a++){
            System.out.println("the number of trees with " + a + " features is " + FeatureConstructionCrossoverPipelineV0.featureNumInOneTree[a]);
        }*/
...
//            System.out.println("1");
        ...
        for (int sub = 0; sub < elitesEveryGen.get(this.generation).length; sub++) {
            for (int ind = 0; ind < elitesEveryGen.get(this.generation)[sub].length; ind++) {
                if (sub == 0) {
                ...
                GPTree tree = ((GPIndividual) (elitesEveryGen.get(this.generation)[sub][ind])).trees[0];
                int nonterminals = tree.child.numNodes(GPNode.NODESEARCH_NONTERMINALS);
...
//                System.out.println(rouFeatureArchive.get(gen).get(i).child.makeLispTree());
    ...
    private void deleteBigRedundantFeatures() {
        ...
        for (int a = 0; a < seqFeatureArchive.get(this.generation).size(); a++) {
            GPTree base = (GPTree) seqFeatureArchive.get(this.generation).get(a).clone();
            for (int b = a + 1; b < seqFeatureArchive.get(this.generation).size(); b++) {
                GPTree count = (GPTree) seqFeatureArchive.get(this.generation).get(b).clone();
                if (FeatureConstructionCrossoverPipeline.LispContains(base.child, count.child)) {
                    seqFeatureArchive.get(this.generation).remove(seqFeatureArchive.get(this.generation).get(a));
                    a--;
                    break;
                }
            }
        ...
        for (int a = 0; a < rouFeatureArchive.get(this.generation).size(); a++) {
            GPTree base = (GPTree) rouFeatureArchive.get(this.generation).get(a).clone();
            for (int b = a + 1; b < rouFeatureArchive.get(this.generation).size(); b++) {
                GPTree count = (GPTree) rouFeatureArchive.get(this.generation).get(b).clone();
                if (FeatureConstructionCrossoverPipeline.LispContains(base.child, count.child)) {
                    rouFeatureArchive.get(this.generation).remove(rouFeatureArchive.get(this.generation).get(a));
                    a--;
                    break;
                }
            }
...
/*        for (int gen1 = 0; gen1 < seqFeatureArchive.size(); gen1++) {
        ...
        for (int a = 0; a < seqFeatureArchive.get(this.generation).size(); a++) {
            ...
            for (int b = a + 1; b < seqFeatureArchive.get(this.generation).size(); b++) {
                GPTree count = (GPTree) seqFeatureArchive.get(this.generation).get(b).clone();
                if (base.child.makeLispTree().equals(count.child.makeLispTree())) {
                    seqFeatureArchive.get(this.generation).remove(seqFeatureArchive.get(this.generation).get(b));
                    b--;
                }
            }
        }
...
/*        for (int gen1 = 0; gen1 < rouFeatureArchive.size(); gen1++) {
        ...
        for (int a = 0; a < rouFeatureArchive.get(this.generation).size(); a++) {
            ...
            for (int b = a + 1; b < rouFeatureArchive.get(this.generation).size(); b++) {
                GPTree count = (GPTree) rouFeatureArchive.get(this.generation).get(b).clone();
                if (base.child.makeLispTree().equals(count.child.makeLispTree())) {
                    rouFeatureArchive.get(this.generation).remove(rouFeatureArchive.get(this.generation).get(b));
                    b--;
                }
            }
        }
