...
public class protectedKozaNodeSelectorV2 extends KozaNodeSelector {
    int pickedNum;
    ...
    public GPNode pickNode(final EvolutionState s,
        ...
        sum++;
        double randomValue = s.random[thread].nextDouble(); //probability  (0,1)

        //只在随机数大于0.5时，才采取保护措施。否则与常规交叉一样
        if (tree.child.depth() > 1 && randomValue > 0.5) {
            ...
            for (int numSubtree = 0; numSubtree < nonterminals; numSubtree++) {
                ...
                if (subpopulation == 0) {
                    for (int gen = 0; gen < ((FreGPRuleEvolutionStateV1)s).seqFeatureArchive.size(); gen++) {
                        for (int f = 0; f < ((FreGPRuleEvolutionStateV1)s).seqFeatureArchive.get(gen).size(); f++) {
                            GPTree countTTree = (GPTree) ((FreGPRuleEvolutionStateV1)s).seqFeatureArchive.get(gen).get(f).clone();
                            if (baseTree.child.makeLispTree().equals(countTTree.child.makeLispTree()))
                                protectedSeqGPTree.add(baseTree);
                        }
                    }
                } else {
                    for (int gen = 0; gen < ((FreGPRuleEvolutionStateV1)s).rouFeatureArchive.size(); gen++) {
                        for (int f = 0; f < ((FreGPRuleEvolutionStateV1)s).rouFeatureArchive.get(gen).size(); f++) {
                            GPTree countTTree = (GPTree) ((FreGPRuleEvolutionStateV1)s).rouFeatureArchive.get(gen).get(f).clone();
                            if (baseTree.child.makeLispTree().equals(countTTree.child.makeLispTree()))
        ...
        } else if (rnd > nonterminalProbability)  // pick terminals  //nonterminalProbability = 0.9
        {
            ...
            for(int a=0; a< terminals; a++){
                nodeNumber.add(a);
            }
            for (int a=0; a< protectedNodeNum.size(); a++){
                if(nodeNumber.contains(protectedNodeNum.get(a))){
                    nodeNumber.remove(protectedNodeNum.get(a));
                    a--;
                }
            }
            int randomNumber = s.random[thread].nextInt(nodeNumber.size());
            pickedNum = nodeNumber.get(randomNumber);
            ...
            if (nonterminals > 0) // there are some nonterminals
            {
                    ...
                    if (subpopulation == 0) {
                        for (int a = 0; a < protectedSeqGPTree.size(); a++) {
                            ...
                            for (int i = begin + 1; i < end + 1; i++) { //加1的原因是因为特征的根节点可以被选中
