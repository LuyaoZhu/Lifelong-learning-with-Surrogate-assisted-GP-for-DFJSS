...
public class protectedKozaNodeSelectorV5 implements GPNodeSelector {
    ...
    public protectedKozaNodeSelectorV5() {
        reset();
    }

    public Object clone() {
        try {
            protectedKozaNodeSelectorV5 s = (protectedKozaNodeSelectorV5) (super.clone());
            s.reset();
            return s;
    ...
    public GPNode pickNode(final EvolutionState s,
        ...
        } else if (rnd > nonterminalProbability)  // pick terminals  //nonterminalProbability = 0.9
        {
            ...
            if (FeatureConstructionCrossoverPipeline.tree1Flag == true) {
                ...
                List tree1list = (List) FeatureConstructionCrossoverPipeline.tree1ProtectedTerminalsRank.stream().distinct().collect(Collectors.toList());
                if (tree1list.size() > 0)
                    pickNodeIndex = (int) tree1list.get(s.random[thread].nextInt(tree1list.size()));
                else
                    pickNodeIndex = s.random[thread].nextInt(terminals);

            } else {

                List tree2list = (List) FeatureConstructionCrossoverPipeline.tree2ProtectedTerminalsRank.stream().distinct().collect(Collectors.toList());
                if (tree2list.size() > 0)
                    pickNodeIndex = (int) tree2list.get(s.random[thread].nextInt(tree2list.size()));
                else
                    pickNodeIndex = s.random[thread].nextInt(terminals);
            }
        ...
        } else  // pick nonterminals if you can
        {
            ...
            if (nonterminals > 0) // there are some nonterminals
            {
                ...
                if (FeatureConstructionCrossoverPipeline.tree1Flag == true) {
                    ...
                    List tree1list = (List) FeatureConstructionCrossoverPipeline.tree1ProtectedNonTerminalsRank.stream().distinct().collect(Collectors.toList());
                    if (tree1list.size() > 0)
                        pickNodeIndex = (int) tree1list.get(s.random[thread].nextInt(tree1list.size()));
                    else
                    ...
                    List tree2list = (List) FeatureConstructionCrossoverPipeline.tree2ProtectedNonTerminalsRank.stream().distinct().collect(Collectors.toList());
                    if (tree2list.size() > 0)
                        pickNodeIndex = (int) tree2list.get(s.random[thread].nextInt(tree2list.size()));
                    else
                        pickNodeIndex = s.random[thread].nextInt(nonterminals);
