...
public class protectedKozaNodeSelectorV3 extends KozaNodeSelector {
    int pickedNum;
        ...
        if (tree.child.depth() > 1) {
            ...
            for (int numSubtree = 0; numSubtree < nonterminals; numSubtree++) {
                ...
                if (subpopulation == 0) {
                        ...
                        for (int f = 0; f < ((FreGPRuleEvolutionStateV3)s).seqFeatureArchive.get(s.generation).size(); f++) {
                            GPTree countTTree = (GPTree) ((FreGPRuleEvolutionStateV3)s).seqFeatureArchive.get(s.generation).get(f).clone();
                        ...
                        for (int f = 0; f < ((FreGPRuleEvolutionStateV3)s).rouFeatureArchive.get(s.generation).size(); f++) {
                            GPTree countTTree = (GPTree) ((FreGPRuleEvolutionStateV3)s).rouFeatureArchive.get(s.generation).get(f).clone();
                            if (baseTree.child.makeLispTree().equals(countTTree.child.makeLispTree()))
