...
public class FeatureConstructionCrossoverPipelineV0Greedy extends GPBreedingPipeline {
    ...
    public static ArrayList<Integer> parent1ProtectedNonTerminalsRank = new ArrayList<>();
    public static ArrayList<Integer> parent1ProtectedTerminalsRank = new ArrayList<>();

    public static ArrayList<Integer> parent2ProtectedNonTerminalsRank = new ArrayList<>();
    public static ArrayList<Integer> parent2ProtectedTerminalsRank = new ArrayList<>();

    public static boolean parent1Flag;

    public static boolean randomSelect;

    public static int[] caseNum = new int[3];  //1 none, 2 only one, 3 both have

    public static int[] case1 = new int[2];
    public static int[] featureNumInOneTree = new int[50];  //
    ...
    public FeatureConstructionCrossoverPipelineV0Greedy() {
    ...
    public Object clone() {
        FeatureConstructionCrossoverPipelineV0Greedy c = (FeatureConstructionCrossoverPipelineV0Greedy) (super.clone());
    ...
    public int produce(final int min,
        ...
        for (int q = start; q < n + start; /* no increment */)  // keep on going until we're filled up
        {
           ...
           /* Produces <i>n</i> individuals from the given subpopulation
            ...
            ArrayList<GPTree> protectedParent1Subtrees = new ArrayList<>();
            ArrayList<GPTree> protectedParent2Subtrees = new ArrayList<>();

            ArrayList<Integer> protectedParent1SubtreesRank = new ArrayList<>();
            ArrayList<Integer> protectedParent2SubtreesRank = new ArrayList<>();
            ...
            if (subpopulation == 0) {
                ...
                for (int parent = 0; parent < parents.length; parent++) {  //for two parents
                    GPTree tree = parents[parent].trees[0];
                    if (tree.child.numNodes(GPNode.NODESEARCH_NONTERMINALS) > 1) {
                        ...
                        for (int numSubtree = 0; numSubtree < nonterminals; numSubtree++) {
                            ...
                            if (parent == 0) {
                                for (int f = 0; f < ((FreGPRuleEvolutionStateV0) state).seqFeatureArchive.get(state.generation).size(); f++) {
                                    GPTree countTree = (GPTree) ((FreGPRuleEvolutionStateV0) state).seqFeatureArchive.get(state.generation).get(f).clone();
                ...
                for (int parent = 0; parent < parents.length; parent++) {  //for two parents
                    ...
                    if (tree.child.numNodes(GPNode.NODESEARCH_NONTERMINALS) > 1) {
                        ...
                        for (int numSubtree = 0; numSubtree < nonterminals; numSubtree++) {
                            ...
                            //compare with the features in the archive
                            if (parent == 0) {
                                for (int f = 0; f < ((FreGPRuleEvolutionStateV0) state).rouFeatureArchive.get(state.generation).size(); f++) {
                                    GPTree countTree = (GPTree) ((FreGPRuleEvolutionStateV0) state).rouFeatureArchive.get(state.generation).get(f).clone();
                                    if (baseTree.child.makeLispTree().equals(countTree.child.makeLispTree())) {
                                        protectedParent1SubtreesRank.add(numSubtree);
                                        protectedParent1Subtrees.add(countTree);
                                    }
...
/*                //record the child terminal nodes
            ...
            if (protectedParent1Subtrees.size() > 0 || protectedParent2Subtrees.size() > 0) {

                //there are only features existing in parent1.
                if (protectedParent1Subtrees.size() > 0 && protectedParent2Subtrees.size() == 0) {
//                    caseNum[1]++;
//                    case1[0]++;
...
//                        p2 = parents[1].trees[t2].child.nodeInPosition(protectedParent2SubtreesRank.get(0), GPNode.NODESEARCH_NONTERMINALS);
                    ...
                    for (int x = 0; x < numTries; x++) {
                        ...
                        if (protectedParent1SubtreesRank.size() == 1) {
                            pickedNodeIndex = protectedParent1SubtreesRank.get(0);
                        } else {
                            int random = state.random[thread].nextInt(protectedParent1SubtreesRank.size());
                            pickedNodeIndex = protectedParent1SubtreesRank.get(random);
                        }
                ...
                else if (protectedParent1Subtrees.size() == 0 && protectedParent2Subtrees.size() > 0) {
//                    caseNum[1]++;
//                    case1[1]++;
                    for (int x = 0; x < numTries; x++) {
                        // pick a node in individual 1
...
/*                        parent1Flag = false;
                        ...
                        int pickedNodeIndex = 0;
                        if (protectedParent2SubtreesRank.size() == 1) {
                            pickedNodeIndex = protectedParent2SubtreesRank.get(0);
                        } else {
                            int random = state.random[thread].nextInt(protectedParent2SubtreesRank.size());
                            pickedNodeIndex = protectedParent2SubtreesRank.get(random);
                        }
                        p2 = parents[1].trees[t2].child.nodeInPosition(pickedNodeIndex, GPNode.NODESEARCH_NONTERMINALS);
                        // check for depth and swap-compatibility limits
                        res1 = verifyPoints(initializer, p2, p1);  // p2 can fill p1's spot -- order is important!
                        if (n - (q - start) < 2 || tossSecondParent) res2 = true;
                        else
...
/*                        int pickedNodeIndex = 0;
                        if (protectedParent2SubtreesRank.size() == 1) {
                            ...
                            int random = state.random[0].nextInt(protectedParent2SubtreesRank.size());
                            pickedNodeIndex = protectedParent2SubtreesRank.get(random);
                        }
...
/*                        parent1Flag = true;
                ...
                parent1ProtectedTerminalsRank.clear();
                parent1ProtectedNonTerminalsRank.clear();
                parent2ProtectedTerminalsRank.clear();
                parent2ProtectedNonTerminalsRank.clear();
...
//                caseNum[0]++;
                ...
                for (int x = 0; x < numTries; x++) {
                    ...
                    res1 = verifyPoints(initializer, p4, p3);  // p2 can fill p1's spot -- order is important!
                    if (n - (q - start) < 2 || tossSecondParent) res2 = true;
                    else res2 = verifyPoints(initializer, p3, p4);  // p1 can fill p2's spot -- order is important!
