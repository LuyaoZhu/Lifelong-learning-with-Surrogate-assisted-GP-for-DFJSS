...
public class protectedKozaNodeSelectorSimplyProtect implements GPNodeSelector {
    ...
    public protectedKozaNodeSelectorSimplyProtect() {
        reset();
    }

    public Object clone() {
        try {
            protectedKozaNodeSelectorSimplyProtect s = (protectedKozaNodeSelectorSimplyProtect) (super.clone());
            s.reset();
            return s;
    ...
    public GPNode pickNode(final EvolutionState s,
        ...
        } else if (rnd > nonterminalProbability)  // pick terminals  //nonterminalProbability = 0.9
        {
            ...
            if (FeatureConstructionCrossoverPipelineSimplyProtect.parent1Flag == true) {
                for (int a = 0; a < FeatureConstructionCrossoverPipelineSimplyProtect.parent1ProtectedTerminalsRank.size(); a++) {
                    if (pickedTerminals.contains(FeatureConstructionCrossoverPipelineSimplyProtect.parent1ProtectedTerminalsRank.get(a)))
                        pickedTerminals.remove(FeatureConstructionCrossoverPipelineSimplyProtect.parent1ProtectedTerminalsRank.get(a));
                }
            } else {
                for (int a = 0; a < FeatureConstructionCrossoverPipelineSimplyProtect.parent2ProtectedTerminalsRank.size(); a++) {
                    if (pickedTerminals.contains(FeatureConstructionCrossoverPipelineSimplyProtect.parent2ProtectedTerminalsRank.get(a)))
                        pickedTerminals.remove(FeatureConstructionCrossoverPipelineSimplyProtect.parent2ProtectedTerminalsRank.get(a));
                }
            }
        ...
        } else  // pick nonterminals if you can
        {
            ...
            if (nonterminals > 0) // there are some nonterminals
            {
                ...
                for (int i = 0; i < nonterminals; i++) {
                    ...
                    if (FeatureConstructionCrossoverPipelineSimplyProtect.parent1Flag == true) {
                        for (int a = 0; a < FeatureConstructionCrossoverPipelineSimplyProtect.parent1ProtectedNonTerminalsRank.size(); a++) {
                            if (pickedNonTerminals.contains(FeatureConstructionCrossoverPipelineSimplyProtect.parent1ProtectedNonTerminalsRank.get(a)))
                                pickedNonTerminals.remove(FeatureConstructionCrossoverPipelineSimplyProtect.parent1ProtectedNonTerminalsRank.get(a));
                        }
                    } else {
                        for (int a = 0; a < FeatureConstructionCrossoverPipelineSimplyProtect.parent2ProtectedNonTerminalsRank.size(); a++) {
                            if (pickedNonTerminals.contains(FeatureConstructionCrossoverPipelineSimplyProtect.parent2ProtectedNonTerminalsRank.get(a)))
                                pickedNonTerminals.remove(FeatureConstructionCrossoverPipelineSimplyProtect.parent2ProtectedNonTerminalsRank.get(a));
                        }
                    }
