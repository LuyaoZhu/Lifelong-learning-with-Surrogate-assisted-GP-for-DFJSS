...
import static yimei.jss.algorithm.featureConstruction.FeatureConstructionCrossoverPipelineV0Greedy.randomSelect;
...
public class protectedKozaNodeSelectorV0Greedy implements GPNodeSelector {
    ...
    public protectedKozaNodeSelectorV0Greedy() {
        reset();
    }

    public Object clone() {
        try {
            protectedKozaNodeSelectorV0Greedy s = (protectedKozaNodeSelectorV0Greedy) (super.clone());
            s.reset();
            return s;
    ...
    public GPNode pickNode(final EvolutionState s,
        ...
        } else if (rnd > nonterminalProbability)  // pick terminals  //nonterminalProbability = 0.9
        {
            ...
            ArrayList<Integer> pickedTerminals = new ArrayList<>();
            for (int i = 0; i < terminals; i++) {
                pickedTerminals.add(i);
            }

            if (randomSelect == true) {

            } else {
                if (FeatureConstructionCrossoverPipelineV0Greedy.parent1Flag == true) {
                    for (int a = 0; a < FeatureConstructionCrossoverPipelineV0Greedy.parent1ProtectedTerminalsRank.size(); a++) {
                        if (pickedTerminals.contains(FeatureConstructionCrossoverPipelineV0Greedy.parent1ProtectedTerminalsRank.get(a)))
                            pickedTerminals.remove(FeatureConstructionCrossoverPipelineV0Greedy.parent1ProtectedTerminalsRank.get(a));
                    }
                } else {
                    for (int a = 0; a < FeatureConstructionCrossoverPipelineV0Greedy.parent2ProtectedTerminalsRank.size(); a++) {
                        if (pickedTerminals.contains(FeatureConstructionCrossoverPipelineV0Greedy.parent2ProtectedTerminalsRank.get(a)))
                            pickedTerminals.remove(FeatureConstructionCrossoverPipelineV0Greedy.parent2ProtectedTerminalsRank.get(a));
                    }
                }
            }
            int pickNodeIndex = 0;
            if (pickedTerminals.size() == 0)
                if (pickedTerminals.size() == 0){
                    pickNodeIndex = s.random[thread].nextInt(terminals);
                    FreGPRuleEvolutionStateV0.zeroNum++;}
                else{
                    pickNodeIndex = pickedTerminals.get(s.random[thread].nextInt(pickedTerminals.size()));
                    FreGPRuleEvolutionStateV0.nonZeroNum++;}

            return tree.child.nodeInPosition(pickNodeIndex, GPNode.NODESEARCH_TERMINALS);
        ...
        } else  // pick nonterminals if you can
        {
            ...
            {
                ArrayList<Integer> pickedNonTerminals = new ArrayList<>();
                for (int i = 0; i < nonterminals; i++) {
                    pickedNonTerminals.add(i);
                }
                if (randomSelect == true) {

                } else {
                    if (FeatureConstructionCrossoverPipelineV0Greedy.parent1Flag == true) {
                        for (int a = 0; a < FeatureConstructionCrossoverPipelineV0Greedy.parent1ProtectedNonTerminalsRank.size(); a++) {
                            if (pickedNonTerminals.contains(FeatureConstructionCrossoverPipelineV0Greedy.parent1ProtectedNonTerminalsRank.get(a)))
                                pickedNonTerminals.remove(FeatureConstructionCrossoverPipelineV0Greedy.parent1ProtectedNonTerminalsRank.get(a));
                        }
                    } else {
                        for (int a = 0; a < FeatureConstructionCrossoverPipelineV0Greedy.parent2ProtectedNonTerminalsRank.size(); a++) {
                            if (pickedNonTerminals.contains(FeatureConstructionCrossoverPipelineV0Greedy.parent2ProtectedNonTerminalsRank.get(a)))
                                pickedNonTerminals.remove(FeatureConstructionCrossoverPipelineV0Greedy.parent2ProtectedNonTerminalsRank.get(a));
                        }
                    }
                }
                int pickNodeIndex = 0;
                if (pickedNonTerminals.size() == 0){
                    pickNodeIndex = s.random[thread].nextInt(nonterminals);
                    FreGPRuleEvolutionStateV0.zeroNum++;}
                else{
                    pickNodeIndex = pickedNonTerminals.get(s.random[thread].nextInt(pickedNonTerminals.size()));
                    FreGPRuleEvolutionStateV0.nonZeroNum++;}

                return tree.child.nodeInPosition(pickNodeIndex, GPNode.NODESEARCH_NONTERMINALS);
                //choose nodes
