...
public class protectedKozaNodeSelectorSimplyProtectNegative implements GPNodeSelector {
    ...
    public protectedKozaNodeSelectorSimplyProtectNegative() {
        reset();
    }

    public Object clone() {
        try {
            protectedKozaNodeSelectorSimplyProtectNegative s = (protectedKozaNodeSelectorSimplyProtectNegative) (super.clone());
            s.reset();
            return s;
    ...
    public GPNode pickNode(final EvolutionState s,
        ...
        } else if (rnd > nonterminalProbability)  // pick terminals  //nonterminalProbability = 0.9
        {
            ...
            if (FeatureConstructionCrossoverPipelineSimplyProtect.parent1Flag == true) {
                if(FeatureConstructionCrossoverPipelineSimplyProtect.parent1ProtectedTerminalsRank.size() == 0){
                    pickNodeIndex = s.random[thread].nextInt(terminals);
                }
                else {
                    int randomValue = s.random[thread].nextInt(FeatureConstructionCrossoverPipelineSimplyProtect.parent1ProtectedTerminalsRank.size());
                        pickNodeIndex = FeatureConstructionCrossoverPipelineSimplyProtect.parent1ProtectedTerminalsRank.get(randomValue);
                }
            }
             else {
                if(FeatureConstructionCrossoverPipelineSimplyProtect.parent2ProtectedTerminalsRank.size() == 0){
                    pickNodeIndex = s.random[thread].nextInt(terminals);
                }
                else {
                    int randomValue = s.random[thread].nextInt(FeatureConstructionCrossoverPipelineSimplyProtect.parent2ProtectedTerminalsRank.size());
                    pickNodeIndex = FeatureConstructionCrossoverPipelineSimplyProtect.parent2ProtectedTerminalsRank.get(randomValue);
                }
            }
        ...
        } else  // pick nonterminals if you can
        {
            ...
            if (nonterminals > 0) // there are some nonterminals
            {
                ...
                if (FeatureConstructionCrossoverPipelineSimplyProtect.parent1Flag == true) {
                    if(FeatureConstructionCrossoverPipelineSimplyProtect.parent1ProtectedNonTerminalsRank.size() == 0){
                        pickNodeIndex = s.random[thread].nextInt(nonterminals);
                    }
                    else {
                        int randomValue = s.random[thread].nextInt(FeatureConstructionCrossoverPipelineSimplyProtect.parent1ProtectedNonTerminalsRank.size());
                        pickNodeIndex = FeatureConstructionCrossoverPipelineSimplyProtect.parent1ProtectedNonTerminalsRank.get(randomValue);
                    }
                }
                else {
                    if(FeatureConstructionCrossoverPipelineSimplyProtect.parent2ProtectedNonTerminalsRank.size() == 0){
                        pickNodeIndex = s.random[thread].nextInt(nonterminals);
                    }
                    else {
                        int randomValue = s.random[thread].nextInt(FeatureConstructionCrossoverPipelineSimplyProtect.parent2ProtectedNonTerminalsRank.size());
                        pickNodeIndex = FeatureConstructionCrossoverPipelineSimplyProtect.parent2ProtectedNonTerminalsRank.get(randomValue);
                    }
                }
