...
public class FeatureConstructionCrossoverPipeline extends GPBreedingPipeline {
    ...
    public static ArrayList<Integer> tree1ProtectedNonTerminalsRank = new ArrayList<>();
    public static ArrayList<Integer> tree1ProtectedTerminalsRank = new ArrayList<>();

    public static ArrayList<Integer> tree2ProtectedNonTerminalsRank = new ArrayList<>();
    public static ArrayList<Integer> tree2ProtectedTerminalsRank = new ArrayList<>();

    public static boolean tree1Flag;
    /**
     * Temporary holding place for parents
     */
    public GPIndividual parents[];

    public FeatureConstructionCrossoverPipeline() {
    ...
    public Object clone() {
        FeatureConstructionCrossoverPipeline c = (FeatureConstructionCrossoverPipeline) (super.clone());
    ...
    public int produce(final int min,
        ...
        for (int q = start; q < n + start; /* no increment */)  // keep on going until we're filled up
        {
           ...
           /* Produces <i>n</i> individuals from the given subpopulation
            ...
            ArrayList<GPTree> protectedGPTree1 = new ArrayList<>();
            ArrayList<GPTree> protectedGPTree2 = new ArrayList<>();

            ArrayList<Integer> protectedGPTree1Rank = new ArrayList<>();
            ArrayList<Integer> protectedGPTree2Rank = new ArrayList<>();

            for (int sub = 0; sub < state.population.subpops.length; sub++) {
                GPTree tree = parents[sub].trees[0];
                if (tree.child.numNodes(GPNode.NODESEARCH_NONTERMINALS) > 1) {
                    ...
                    for (int numSubtree = 0; numSubtree < nonterminals; numSubtree++) {
                        ...
                        if (sub == 0) {
                            for (int f = 0; f < ((FreGPRuleEvolutionStateV3) state).seqFeatureArchive.get(state.generation).size(); f++) {
                                GPTree countTree = (GPTree) ((FreGPRuleEvolutionStateV3) state).seqFeatureArchive.get(state.generation).get(f).clone();
                                if (baseTree.child.makeLispTree().equals(countTree.child.makeLispTree())) {
                                    protectedGPTree1Rank.add(numSubtree);
                                    protectedGPTree1.add(countTree);
                                }
                            }
                        } else {
                            for (int f = 0; f < ((FreGPRuleEvolutionStateV3) state).rouFeatureArchive.get(state.generation).size(); f++) {
                                GPTree countTree = (GPTree) ((FreGPRuleEvolutionStateV3) state).rouFeatureArchive.get(state.generation).get(f).clone();
                                if (baseTree.child.makeLispTree().equals(countTree.child.makeLispTree())) {
                                    protectedGPTree2Rank.add(numSubtree);
                                    protectedGPTree2.add(countTree);
                                }
                            }
                        }
                    }
            ...
            //we need to record the rank of protected
            if (protectedGPTree1.size() > 0) {
                int fullTreeDepth = parents[0].trees[0].child.depth();
                for (int i = 0; i < protectedGPTree1Rank.size(); i++) {
                    int subtreeNonterminals = protectedGPTree1.get(i).child.numNodes(GPNode.NODESEARCH_NONTERMINALS);

                    //record the child nonTerminal nodes
                    for (int num = protectedGPTree1Rank.get(i); num < protectedGPTree1Rank.get(i) + subtreeNonterminals; num++) {
                        tree1ProtectedNonTerminalsRank.add(num);
                    }
                    //record the parents nodes
                    int featureDepth = protectedGPTree1.get(i).child.depth();
...
/*                    for (int depth = fullTreeDepth; depth > featureDepth; depth--) {
                        for (int numSubtree = 0; numSubtree < fulltreeNonterminals; numSubtree++) {
                            ...
                            if (baseTree.child.depth() == depth) {
                                for (int x = 0; x < baseTree.child.children.length; x++) {
                                    if (baseTree.child.children[x].makeLispTree().equals(protectedGPTree1.get(i).child.makeLispTree())) {
                                        tree1ProtectedNonTerminalsRank.add(numSubtree);
                                    }
                                }
                            }
                        }
                    ...
                    for (int depth = fullTreeDepth; depth > featureDepth; depth--) {
                        for (int numSubtree = 0; numSubtree < fulltreeNonterminals; numSubtree++) {
                            ...
                            if (node.depth() == depth) {
                                if (LispContains(node, protectedGPTree1.get(i).child)) {
                                    tree1ProtectedNonTerminalsRank.add(numSubtree);
                                }
                            }
                        }
                    ...
                    int subtreeTerminals = protectedGPTree1.get(i).child.numNodes(GPNode.NODESEARCH_TERMINALS);
                    ...
                    for (int a = 0; a < fulltreeTerminals; a++) {
                        ...
                        for (int b = 0; b < subtreeTerminals; b++) {
                            ...
                            GPNode countNode = protectedGPTree1.get(i).child.nodeInPosition(b, GPNode.NODESEARCH_TERMINALS);
                        ...
                        if (sameNum == subtreeTerminals) {
                            for (int num = a; num < a + subtreeTerminals; num++) {
                                tree1ProtectedTerminalsRank.add(num);
                            }
                            break;
                        }
                    }
...
//                    for (int b = 0; b < protectedGPTree1.size(); b++) {
//                        if (LispContains(protectedGPTree1.get(b).child, baseNode)) {
//                            int depth = protectedGPTree1.get(b).child.depth();
//                            GPNode tempNode = (GPNode) baseNode.clone();
//                            for (int d = 0; d < depth - 1; d++) {
//                                tempNode = (GPNode) tempNode.parent;
//                            }
//                            if (tempNode.makeLispTree().equals(protectedGPTree1.get(b).child.makeLispTree())) {
//                                tree1ProtectedTerminalsRank.add(a);
//                                break;
//                            }
//                        }
//                    }
//                }
            ...
            if (protectedGPTree2.size() > 0) {

                int fullTreeDepth = parents[1].trees[0].child.depth();
                for (int i = 0; i < protectedGPTree2Rank.size(); i++) {
                    int subtreeNonterminals = protectedGPTree2.get(i).child.numNodes(GPNode.NODESEARCH_NONTERMINALS);

                    //record the child nonTerminal nodes
                    for (int num = protectedGPTree2Rank.get(i); num < protectedGPTree2Rank.get(i) + subtreeNonterminals; num++) {
                        tree2ProtectedNonTerminalsRank.add(num);
                    }
                    //record the parents nodes
                    int featureDepth = protectedGPTree2.get(i).child.depth();
...
//                            if (baseTree.child.depth() == depth) {
//                                for (int x = 0; x < baseTree.child.children.length; x++) {
//                                    if (baseTree.child.children[x].makeLispTree().equals(protectedGPTree2.get(i).child.makeLispTree())) {
//                                        tree2ProtectedNonTerminalsRank.add(numSubtree);
//                                    }
//                                }
//                            }
//                        }
//                    }
                    ...
                    for (int depth = fullTreeDepth; depth > featureDepth; depth--) {
                        for (int numSubtree = 0; numSubtree < fulltreeNonterminals; numSubtree++) {
                            ...
                            if (node.depth() == depth) {
                                if (LispContains(node, protectedGPTree2.get(i).child)) {
                                    tree2ProtectedNonTerminalsRank.add(numSubtree);
                                }
                            }
                        }
                    ...
                    //record the child terminal nodes
                    int subtreeTerminals = protectedGPTree2.get(i).child.numNodes(GPNode.NODESEARCH_TERMINALS);
                    ...
                    for (int a = 0; a < fulltreeTerminals; a++) {
                        ...
                        for (int b = 0; b < subtreeTerminals; b++) {
                            ...
                            GPNode countNode = protectedGPTree2.get(i).child.nodeInPosition(b, GPNode.NODESEARCH_TERMINALS);
                            if (LispContains(baseNode, countNode)) {
                                sameNum++;
                            }
                        }
                        if (sameNum == subtreeTerminals) {
                            for (int num = a; num < a + subtreeTerminals; num++) {
                                tree2ProtectedTerminalsRank.add(num);
                            }
                            break;
                        }
                    }
...
/*                //record the child terminal nodes
                for (int a = 0; a < parents[1].trees[0].child.numNodes(GPNode.NODESEARCH_TERMINALS); a++) {
                    ...
                    for (int b = 0; b < protectedGPTree2.size(); b++) {
                        if (LispContains(protectedGPTree2.get(b).child, baseNode)) {
                            int depth = protectedGPTree2.get(b).child.depth();
                            GPNode tempNode = (GPNode) baseNode.clone();
                            for (int d = 0; d < depth - 1; d++) {
                                tempNode = (GPNode) tempNode.parent;
                            }
                            if (tempNode.makeLispTree().equals(protectedGPTree2.get(b).child.makeLispTree())) {
                                tree2ProtectedTerminalsRank.add(a);
                                break;
                            }
                        }
                    }
            ...
            GPNode p3 = null;      // for ind1
            GPNode p4 = null;      // for ind2


            if (protectedGPTree1.size() > 0 || protectedGPTree2.size() > 0) {



                //there are only features existing in tree1.
                if (protectedGPTree1.size() > 0 && protectedGPTree2.size() == 0) {
...
//                        p2 = parents[1].trees[t2].child.nodeInPosition(protectedGPTree2Rank.get(0), GPNode.NODESEARCH_NONTERMINALS);
                    ...
                    for (int x = 0; x < numTries; x++) {
                        // pick a node in individual 1
//                    p3 = nodeselect1.pickNode(state, subpopulation, thread, parents[0], parents[0].trees[t1]);
                        int pickedNodeIndex = 0;
                        if (protectedGPTree1Rank.size() == 1) {
                            pickedNodeIndex = protectedGPTree1Rank.get(0);
                        } else {
                            int random = state.random[thread].nextInt(protectedGPTree1Rank.size());
                            pickedNodeIndex = protectedGPTree1Rank.get(random);
                        }

                        p3 = parents[0].trees[t1].child.nodeInPosition(pickedNodeIndex, GPNode.NODESEARCH_NONTERMINALS);
                        // pick a node in individual 2
                        tree1Flag = false;
                        p4 = nodeselect2.pickNode(state, subpopulation, thread, parents[1], parents[1].trees[t2]);

                        // check for depth and swap-compatibility limits
                        res1 = verifyPoints(initializer, p4, p3);  // p2 can fill p1's spot -- order is important!
                        if (n - (q - start) < 2 || tossSecondParent) res2 = true;
                        else
                            res2 = verifyPoints(initializer, p3, p4);  // p1 can fill p2's spot -- order is important!
                ...
                //there are only features existing in tree2.
                else if (protectedGPTree1.size() == 0 && protectedGPTree2.size() > 0) {
...
//                        p3 = parents[0].trees[t1].child.nodeInPosition(pickedNodeIndex, GPNode.NODESEARCH_NONTERMINALS);
                        ...
                        int pickedNodeIndex = 0;
                        if (protectedGPTree2Rank.size() == 1) {
                            pickedNodeIndex = protectedGPTree2Rank.get(0);
                        } else {
                            int random = state.random[0].nextInt(protectedGPTree2Rank.size());
                            pickedNodeIndex = protectedGPTree2Rank.get(random);
                        }

                        p4 = parents[1].trees[t1].child.nodeInPosition(pickedNodeIndex, GPNode.NODESEARCH_NONTERMINALS);
//                        p4 = nodeselect2.pickNode(state, subpopulation, thread, parents[1], parents[1].trees[t2]);
                ...
                // both tree2 have features.
                    ...
                    for (int x = 0; x < numTries; x++) {
                        ...
                        p2 = parents[1].trees[t1].child.nodeInPosition(pickedNodeIndex, GPNode.NODESEARCH_NONTERMINALS);
//                        p2 = nodeselect2.pickNode(state, subpopulation, thread, parents[1], parents[1].trees[t2]);
//                        p2 = parents[1].trees[t2].child.nodeInPosition(protectedGPTree2Rank.get(0), GPNode.NODESEARCH_NONTERMINALS);
                        // check for depth and swap-compatibility limits
                        res1 = verifyPoints(initializer, p2, p1);  // p2 can fill p1's spot -- order is important!
...
//                        p3 = parents[0].trees[t1].child.nodeInPosition(pickedNodeIndex, GPNode.NODESEARCH_NONTERMINALS);
                ...
                tree1ProtectedTerminalsRank.clear();
                tree1ProtectedNonTerminalsRank.clear();
                tree2ProtectedTerminalsRank.clear();
                tree2ProtectedNonTerminalsRank.clear();
                ...
                if (n - (q - start) >= 2 && !tossSecondParent)
                    for (int x = 0; x < j2.trees.length; x++) {
                        if (x == t2 && res2)  // we've got a tree with a kicking cross position!
                        {
                            ...
                            j2.trees[x].owner = j2;
                            j2.trees[x].child = parents[1].trees[x].child.cloneReplacing(p3, p4);
            ...
            // Both two individuals do not have features.
            else {
                for (int x = 0; x < numTries; x++) {
                    // pick a node in individual 1
                    tree1Flag = true;
