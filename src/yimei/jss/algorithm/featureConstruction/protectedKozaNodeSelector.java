...
public class protectedKozaNodeSelector extends KozaNodeSelector {
    int pickedNum;
//    public int useTime;
    @Override
    public GPNode pickNode(final EvolutionState s,
        ...
        if (tree.child.depth() > 1) {
            ...
            for (int numSubtree = 0; numSubtree < nonterminals; numSubtree++) {
                ...
                if (subpopulation == 0) {
                    for (int gen = 0; gen < ((FreGPRuleEvolutionState)s).seqFeatureArchive.size(); gen++) {
                        for (int f = 0; f < ((FreGPRuleEvolutionState)s).seqFeatureArchive.get(gen).size(); f++) {
                            GPTree countTTree = (GPTree) ((FreGPRuleEvolutionState)s).seqFeatureArchive.get(gen).get(f).clone();
                            if (baseTree.child.makeLispTree().equals(countTTree.child.makeLispTree()))
                                protectedSeqGPTree.add(baseTree);
                        }
                    }
                } else {
                    for (int gen = 0; gen < ((FreGPRuleEvolutionState)s).rouFeatureArchive.size(); gen++) {
                        for (int f = 0; f < ((FreGPRuleEvolutionState)s).rouFeatureArchive.get(gen).size(); f++) {
                            GPTree countTTree = (GPTree) ((FreGPRuleEvolutionState)s).rouFeatureArchive.get(gen).get(f).clone();
                            if (baseTree.child.makeLispTree().equals(countTTree.child.makeLispTree()))
        ...
        } else if (rnd > nonterminalProbability)  // pick terminals  //nonterminalProbability = 0.9
        {
            ...
            if(s.generation%5 ==1) {
                ...
                pickedNum = s.random[thread].nextInt(terminals);
