...
public class FeatureConstructionMutationPipeline extends GPBreedingPipeline {
    ...
    public Object clone() {
        FeatureConstructionMutationPipeline c = (FeatureConstructionMutationPipeline) (super.clone());
...
/*                int size = GPNodeBuilder.NOSIZEGIVEN;
                ...
                if (subpopulation == 0) {
                    int num = ((FreGPRuleEvolutionStateV0)state).seqFeatureArchive.get(state.generation).size();
                    int randomValue = state.random[0].nextInt(num);
                    p2 = ((FreGPRuleEvolutionStateV0)state).seqFeatureArchive.get(state.generation).get(randomValue).child;
                } else {
                    int num = ((FreGPRuleEvolutionStateV0)state).rouFeatureArchive.get(state.generation).size();
                    int randomValue = state.random[0].nextInt(num);
                    p2 = ((FreGPRuleEvolutionStateV0)state).rouFeatureArchive.get(state.generation).get(randomValue).child;
                }
