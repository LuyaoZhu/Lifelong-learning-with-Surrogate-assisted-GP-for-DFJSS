 ...
 *
 * save elites from different tasks, all is 5%
 *
 * @author luyao
 */

public class GPRuleEvolutionStateLifelongGPV11 extends GPRuleEvolutionStateLifelongGPV3 {
    ...
    ArrayList<ArrayList<Individual>> elitesInTasks = new ArrayList<>();
...
//        System.out.println(diversityValue[0]);
        ...
        if (generation == generationPerTask - 1 || generation == generationPerTask * 2 - 1 || generation == generationPerTask * 3 - 1) {
            ...
            ArrayList<Individual> elitesInCurrentTask = new ArrayList<>();
            PopulationUtils.sort(population);
            for (int i=0; i< (int)(population.subpops[0].individuals.length * seedingRatio); i++){
                elitesInCurrentTask.add((Individual) population.subpops[0].individuals[i].clone());
            }
            elitesInTasks.add(elitesInCurrentTask);
...
//            writeFirstSelectedCasesNumToFile(firstSelectedCasesNum);
        ...
        if (generation == generationPerTask - 1 || generation == generationPerTask * 2 - 1 || generation == generationPerTask * 3 - 1) {
            ...
            int numIndividuals = (int) (population.subpops[0].individuals.length * seedingRatio);
            int numIndividualsPerTask = (int)(numIndividuals / elitesInTasks.size());

            ArrayList<Individual> savedTopIndividuals = new ArrayList<>();

            for (int t = 0; t < elitesInTasks.size(); t++) {
                for (int i=0; i<numIndividualsPerTask; i++) {
                    savedTopIndividuals.add((Individual) elitesInTasks.get(t).get(i).clone());
                }
            }
