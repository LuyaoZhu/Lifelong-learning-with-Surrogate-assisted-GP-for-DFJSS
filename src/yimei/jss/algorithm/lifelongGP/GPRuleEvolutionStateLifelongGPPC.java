...
public class GPRuleEvolutionStateLifelongGPPC extends GPRuleEvolutionStateLifelongGPV0 {
    ...
    ArrayList<int[]> bestIndividualPC = new ArrayList<>();
...
//        System.out.println(diversityValue[0]);
        ...
        for (Individual individual:population.subpops[0].individuals){
            if(((GPIndividual)individual).fromLastTask == true){
                fromLastTaskNumber++;
            }
        }

        fromLastTaskRatio.add(fromLastTaskNumber/population.subpops[0].individuals.length);
        ...
        if(minCurrentTask < refFit) {
            for (int ind = 0; ind < population.subpops[0].individuals.length; ind++) {
                ...
                if (individual.fitness.fitness() < Double.MAX_VALUE) {
                    ...
                    objective[0] = (individual.fitness.fitness() - minCurrentTask) / (refFit - minCurrentTask);
...
//            writeFirstSelectedCasesNumToFile(firstSelectedCasesNum);
        ...
        if (generation >= generationPerTask) {
            ...
            for (int ind = 0; ind < population.subpops[0].individuals.length; ind++) {
                ...
                for (int PC=0; PC<bestIndividualPC.size();PC++) {
                    double referenceDistance = PhenoCharacterisation.distance(bestIndividualPC.get(PC), referencePC);
                    PCGap += PhenoCharacterisation.distance(bestIndividualPC.get(PC), individual.PC)/referenceDistance;
                }

                PCGap /= bestIndividualPC.size();

                System.out.print( PCGap+" , ");

                double[] objective = new double[1];
                objective[0] = individual.fitness.fitness() + 0.3 * PCGap;
        ...
        if(generation == generationPerTask-1 || generation == generationPerTask*2-1  || generation == generationPerTask*3-1 ){
            ...
            bestIndividualPC.add(((GPIndividual)population.subpops[0].individuals[0]).PC);

            System.out.println(Arrays.toString(((GPIndividual)population.subpops[0].individuals[0]).PC));
