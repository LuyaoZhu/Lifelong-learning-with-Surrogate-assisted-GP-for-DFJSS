...
public class GPRuleEvolutionStateLifelongGPV10N extends GPRuleEvolutionStateLifelongGPV3 {
...
//        System.out.println(diversityValue[0]);
        ...
        double refFit = calculateReferenceRuleFitness(simulation); //this is for normalising individuals' fitness
        ...
        if (generation >= generationPerTask) {
            ...
            double[] minFitness = new double[surrogateSamples.size() + 1];
...
//            double[] mediumPCDistance = calculateMediumPCDistance(surrogateSamples);
            ...
            double min = Arrays.stream(estimatedFitness[estimatedFitness.length - 1]).min().getAsDouble();

            if(min < refFit){  //this means GP individuals are better than reference rule
                //max-min normalise individuals
                normalisePopulation(min, refFit);
...
//                System.out.println(Arrays.stream(estimatedFitness[t]).min().getAsDouble());
                    ...
                    minFitness[t] = (Arrays.stream(surrogateFitness.get(t)).min().getAsDouble() - minInPreviousTask) / (1 - minInPreviousTask);
                }
...
//                thresholds = calculateThresholdsV3(estimatedFitness, minFitness, thresholdsEveryGeneration.get(thresholdsEveryGeneration.size() - 1));
                double[] a = new double[thresholds.length];
                Arrays.fill(a, 1.0);
                thresholds = calculateThresholdsV3(estimatedFitness, minFitness, a);
            }
...
// INCREMENT GENERATION AND CHECKPOINT
    ...
    private void normalisePopulation(double min, double refFit) {
...
//        }
    ...
    private double[] calculateThresholdsV3(
        ...
        // =================================================
        // Step 0: 计算每个个体的加权 fitness
        // =================================================
        class Individual {
            int index;
            double weightedFitness;

            Individual(int index, double weightedFitness) {
                this.index = index;
        ...
        for (int j = 0; j < N; j++) {
            ...
            if (wSum > 0) {
                individuals.add(new Individual(j, sum / wSum));
            }
        }

        // 按 fitness 从小到大（小的好）
        individuals.sort(Comparator.comparingDouble(ind -> ind.weightedFitness));


        HashSet<Integer> selectedIndividuals = new HashSet<>();
        for (int k = 0; k < individuals.size(); k++) {
            if(individuals.get(k).weightedFitness < 999) {
                selectedIndividuals.add(individuals.get(k).index);
            }
        }
        ...
        if(selectedIndividuals.size() > 1) {
            ...
            for (int i = 0; i < T - 1; i++) {
                ...
                double mean = fitnessInOneTask.isEmpty()
                        ? Double.NaN
                        : sum / fitnessInOneTask.size();
            ...
            for (int i = 0; i < thresholds.length; i++) {
                if (i == thresholds.length - 1) {
                    ...
                    thresholds[i] = 1/(thresholds.length-1);
                }
            }
        }
...
//
    ...
    public class ConvergenceChecker {
        ...
        private double improvementThreshold = 0.01; // 0.1%
