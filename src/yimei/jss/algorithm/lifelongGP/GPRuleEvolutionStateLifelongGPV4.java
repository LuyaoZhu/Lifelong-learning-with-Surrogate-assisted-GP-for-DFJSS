...
import yimei.jss.algorithm.surrogateAccuracy.surrogateClearingMultitreeEvaluatorV1;
...
/**
 * in each task, select individuals with good quality and diversity,
 * and evaluate them with 10 unseen instances in the final generation to build surrogate
 *
 * @author luyao
 */

public class GPRuleEvolutionStateLifelongGPV4 extends GPRuleEvolutionStateLifelongGPV3 {
...
//        System.out.println(diversityValue[0]);
        ...
        for (int ind = 0; ind < population.subpops[0].individuals.length; ind++) {
            ...
            if (individual.fitness.fitness() < 0.6) {
...
// -------------------------------
            ...
            int[][] PCs = new int[qdEntries.size()][];
...
// then evaluate them and build surrogate

            double[] fitnessOneSurrogate = new double[QDIndividuals.size()];

            for (int s = 0; s < simulationsPerTask; s++) {

                simulation.reset(608 + s);
                double[] refFit1 = calculateReferenceRuleFitness(simulation);
                ...
                for (int ind = 0; ind < QDIndividuals.size(); ind++) {

                    GPIndividual indi = QDIndividuals.get(ind);
                    ...
                    // normalise
                    fitnessOneSurrogate[ind] += ObjValue / refFit1[1];
                }
            }
            ...
            surrogateSamples.add(PCs);   // ✅ 这里 PCs[ind] 与 QDIndividuals.get(ind) 严格一一对应
        ...
        if (generation >= generationPerTask) {
            ...
            for (int t = 0; t < surrogateSamples.size(); t++) {
                estimatedFitness[t] = evaluatePopulation(this, surrogateSamples.get(t), surrogateFitness.get(t), 8);
            }
