...
/**
 * incrementally increase the surrogate sample from the beginning
 * normalize use two reference rules
 *
 * @author luyao
 */

public class GPRuleEvolutionStateLifelongGPV3 extends GPRuleEvolutionStateLifelongGPV0 {
    ...
    public ArrayList<int[][]> surrogateSamples = new ArrayList<>();
    public ArrayList<double[]> surrogateFitness = new ArrayList<>();

    HashMap<List<Integer>, List<Double>> PCFitnessMap = new HashMap<>();

    ArrayList<Integer> surrogateSampleNumber = new ArrayList<>();
...
//        System.out.println(diversityValue[0]);
        ...
        //in addition, we also use two reference rule pair (i.e., WIQ+SPT, WIQ+WSPT) to obtain the reference fitness

        double[] refFit = calculateReferenceRuleFitness(simulation);

//        double bestBase  = Math.min(refFit[0], refFit[1]);
//        double worstBase = Math.max(refFit[0], refFit[1]);
...
//                objective[0] = (individual.fitness.fitness() - bestBase) / (worstBase - bestBase);
...
//        }
        ...
        for (int ind = 0; ind < population.subpops[0].individuals.length; ind++) {
            ...
            if (individual.fitness.fitness() < Double.MAX_VALUE) {
                double[] objective = new double[1];
                objective[0] = individual.fitness.fitness()  / refFit[1];
                ((MultiObjectiveFitness) individual.fitness).setObjectives(this, objective);
            }
        }
        ...
        //put it into hashMap
        for (int s = 0; s < indsCharListsMultiTreeCopy.length; s++) {

            int[] raw = indsCharListsMultiTreeCopy[s];
            double fitness = fitnessesForModel[s];

            // ✅ 把 int[] 变成按内容比较的 key
            List<Integer> key = Arrays.stream(raw)
                    .boxed()
                    .collect(Collectors.toList());

            // ✅ 一个 PC 自动对应一组 fitness
            PCFitnessMap
                    .computeIfAbsent(key, k -> new ArrayList<>())
                    .add(fitness);
        }

        System.out.println("The number of surrogate sample in task "+ (generation/generationPerTask+1) + " is " +PCFitnessMap.size());

        surrogateSampleNumber.add(PCFitnessMap.size());

        //at the end of one task, we will build a surrogate
        if(generation == generationPerTask-1 || generation == generationPerTask*2-1  || generation == generationPerTask*3-1 ){

//            PopulationUtils.sort(population); //to select the best individual to get PC
//            phenoCharacterisation[0] = SequencingPhenoCharacterisation.currentBestRulePhenoCharacterisation(this, problem);
//            phenoCharacterisation[1] = RoutingPhenoCharacterisation.currentBestRulePhenoCharacterisation(this, problem);
//            int[][][] indsCharListsMultiTree = phenotypicForSurrogate.muchBetterPhenotypicPopulationRanks(this,phenoCharacterisation);

            int[][] indsCharListsMultiTreeOneTask = new int[PCFitnessMap.size()][indsCharListsMultiTree[0].length];

            double[] fitnessOneTask = new double[PCFitnessMap.size()];

            int idx = 0;

            for (Map.Entry<List<Integer>, List<Double>> entry : PCFitnessMap.entrySet()) {

                List<Integer> pc = entry.getKey();
                List<Double> fitnessList = entry.getValue();

                // ✅ 1. 把 List<Integer> -> int[]
                for (int j = 0; j < pc.size(); j++) {
                    indsCharListsMultiTreeOneTask[idx][j] = pc.get(j);
                }

                // ✅ 2. 计算该 PC 的平均 fitness
                double avgFitness = fitnessList.stream()
                        .mapToDouble(x -> x)
                        .average()
                        .orElse(0.0);

                fitnessOneTask[idx] = avgFitness;
            ...
            surrogateSamples.add(indsCharListsMultiTreeOneTask);
            surrogateFitness.add(fitnessOneTask);
        }
        ...
        if(generation >= generationPerTask){
            ...
            for (int t=0; t< surrogateSamples.size(); t++){
                estimatedFitness[t] = evaluatePopulation(this,surrogateSamples.get(t),surrogateFitness.get(t));
            }
        ...
        if (generation == numGenerations - 1) {
            ...
            writeSurrogateSampleNumToFile(surrogateSampleNumber);

            writeSurrogatePCDistanceToFile(averagePCDistance);
...
// INCREMENT GENERATION AND CHECKPOINT
    ...
    private void writeSurrogatePCDistanceToFile(ArrayList<Double> averagePCDistance) {
        // fzhang 2019.5.21 save the number of cleared individuals
        File weightFile = new File(out_dir + "/job." + jobSeed + ".averagePCDistance.csv"); // jobSeed = 0
        try {
            ...
            if (!averagePCDistance.isEmpty()) {
                header.append(",averagePCDistance");
            ...
            for (int i = 0; i < averagePCDistance.size(); i++) {
                StringBuilder line = new StringBuilder(i+generationPerTask + "");
                line.append(", ").append(averagePCDistance.get(i));
    ...
    private void writeSurrogateSampleNumToFile(ArrayList<Integer> surrogateSampleNumber) {
            // fzhang 2019.5.21 save the number of cleared individuals
            File weightFile = new File(out_dir + "/job." + jobSeed + ".surrogateSampleNum.csv"); // jobSeed = 0
            try {
                BufferedWriter writer = new BufferedWriter(new FileWriter(weightFile));

                // Dynamically create the header based on the size of entropyDiversity elements
                StringBuilder header = new StringBuilder("Gen");
                if (!surrogateSampleNumber.isEmpty()) {
                        header.append(",surrogateSampleNum");
                ...
                // Write the data
                for (int i = 0; i < surrogateSampleNumber.size(); i++) {
                    StringBuilder line = new StringBuilder(i + "");
                    line.append(", ").append(surrogateSampleNumber.get(i));
                    writer.write(line.toString());
                    writer.newLine();
                }

                surrogateSampleNumber.clear();
                writer.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
...
//                        ((Clearable)individual.fitness).surrogateFitness(fitnessesForModel[index]);
        ...
        PCDistance /= population.subpops[0].individuals.length;

        averagePCDistance.add(PCDistance);
