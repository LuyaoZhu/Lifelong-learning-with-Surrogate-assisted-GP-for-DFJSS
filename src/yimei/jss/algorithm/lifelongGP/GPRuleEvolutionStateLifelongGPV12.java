...
import static yimei.jss.algorithm.surrogateAccuracy.surrogateClearingMultitreeEvaluatorV1.*;
...
public class GPRuleEvolutionStateLifelongGPV12 extends GPRuleEvolutionStateLifelongGPV3 {
...
//        System.out.println(diversityValue[0]);
        ...
        if (generation >= generationPerTask) {
            ...
            for (int j = 0; j < population.subpops[0].individuals.length; j++) {
                Individual ind = (Individual) population.subpops[0].individuals[j];
                if (ind.fitness.fitness() < Double.MAX_VALUE) {
                    fitnessInOneTask.add(ind.fitness.fitness());
                }
            }
            ...
            averageFitness.add(mean);

            if(generation%generationPerTask == 0){
                coefficientCurrentTask = 0.5;
            } else {
                coefficientCurrentTask = updateTaskWeight(averageFitness.get(averageFitness.size() - 2), averageFitness.get(averageFitness.size() - 1), coefficientCurrentTask);
            }
            ...
            thresholdsEveryGeneration.add(coefficientCurrentTask);

            System.out.println("The threshold of Task in Generation" + generation + " is " + coefficientCurrentTask);
            ...
            if(generation%generationPerTask == 99){
    ...
    public void writeThresholdsToFile(ArrayList<Double> thresholdsEveryGen) {
        ...
        try {
            ...
            for (int i = 0; i < thresholdsEveryGen.size(); i++) {
                ...
                line.append(", ").append(thresholdsEveryGen.get(i));
...
//                        ((Clearable)individual.fitness).surrogateFitness(fitnessesForModel[index]);
    ...
    public static double updateTaskWeight(
            double prevFitness,
            double currFitness,
            double prevWeight) {
        ...
        // Hyper-parameters
        // =========================
        final double ETA = 1.0;        // sensitivity of weight update
        final double GAMMA = 3.0;      // amplification for performance degradation
        final double EPSILON = 0.005;  // stagnation threshold
        final double BETA = 0.5;       // penalty for stagnation
        final double W_MIN = 0.1;      // lower bound of weight
        final double W_MAX = 0.9;      // upper bound of weight

        // =========================
        // Step 1: learning progress
        // =========================
        double delta = currFitness - prevFitness;

        // =========================
        // Step 2: progress response
        // =========================
        double progressScore;

        if (delta < 0) {
            // Performance degradation: strongly increase weight
            progressScore = GAMMA * Math.abs(delta);
        } else if (delta < EPSILON) {
            // Stagnation: decrease weight
            progressScore = -BETA;
        } else {
            // Normal improvement: increase weight
            progressScore = delta;
        }
        ...
        // =========================
        double newWeight = prevWeight * Math.exp(ETA * progressScore);
