...
public class GPRuleEvolutionStateLifelongGPMultiTask extends GPRuleEvolutionStateLifelongGPV0 {
...
//        System.out.println(diversityValue[0]);
        ...
        refFit = calculateReferenceRuleFitness(((MultipleTreeMultipleRuleEvaluationModel) problem.getEvaluationModel()).getSchedulingSet().getSimulations());
...
// INCREMENT GENERATION AND CHECKPOINT
    ...
    private double[] calculateReferenceRuleFitness(List<Simulation> simulations) {
        // 两个参考 rule 组合：SPT+WIQ 和 WSPT+WIQ
        AbstractRule referenceSeqRule = new WSPT(RuleType.SEQUENCING);
        AbstractRule referenceRouRule = new WIQ(RuleType.ROUTING);

        double[] fitness = new double[simulations.size()];
        ...
        for (int s = 0; s < simulations.size(); s++) {
            fitness[s] = evaluateRulePair((DynamicSimulation) simulations.get(s), referenceSeqRule, referenceRouRule, objective);
        }
