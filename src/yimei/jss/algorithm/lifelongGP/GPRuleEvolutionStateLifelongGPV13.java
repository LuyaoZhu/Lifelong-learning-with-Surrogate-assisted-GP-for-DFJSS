 ...
 *
 * !!!!!!!!fitness is max-min normalised
 *
 * @author luyao
 */

public class GPRuleEvolutionStateLifelongGPV13 extends GPRuleEvolutionStateLifelongGPV3 {
...
//                System.out.println(Arrays.stream(estimatedFitness[t]).min().getAsDouble());
            ...
            estimatedFitness[estimatedFitness.length - 1] = minMaxNormalize(estimatedFitness[estimatedFitness.length - 1]);
...
//                thresholds = calculateThresholdsV2(estimatedFitness);
//            }
...
//                        state.output.warning("Bad objective #" + ": " + objective[0] + ", setting to worst value for that objective.");
        ...
        if (generation == generationPerTask - 1 || generation == generationPerTask * 2 - 1 || generation == generationPerTask * 3 - 1) {
            ...
            //to max-min normalise the fitness
            fitnessOneSurrogate = minMaxNormalize(fitnessOneSurrogate);
...
// INCREMENT GENERATION AND CHECKPOINT
    ...
    private double[] calculateThresholdsV2(double[][] estimatedFitness) {
...
//            thresholds[i] = thresholds[i]/variance[i];
    ...
    private double[] calculateThresholdsV3(double[][] estimatedFitness) {

        double[] thresholds = new double[estimatedFitness.length];

        double sumThreshold = 0;

        for (int i = 0; i < estimatedFitness.length-1; i++) {
            ArrayList<Double> fitnessInOneTask = new ArrayList<>();
            for (int j = 0; j < estimatedFitness[i].length; j++) {
                if (estimatedFitness[i][j] < Double.MAX_VALUE) {
                    fitnessInOneTask.add(estimatedFitness[i][j]);
...
// 生成排名数组
    ...
    private static double[][] minMaxNormalize(double[][] fitness) {
        int groups = fitness.length;
        int dim = fitness[0].length;

        double[][] normalized = new double[groups][dim];

        for (int i = 0; i < groups; i++) {
            double min = Double.MAX_VALUE;
            double max = -Double.MAX_VALUE;

            // 1. 计算该组 min / max（忽略 Double.MAX_VALUE）
            for (int j = 0; j < dim; j++) {
                double val = fitness[i][j];
            ...
            for (int j = 0; j < dim; j++) {
                ...
                if (val == Double.MAX_VALUE) {
                    normalized[i][j] = Double.MAX_VALUE;
                } else {
                    if (max > min) {
                        normalized[i][j] = (val - min) / (max - min);
                    } else {
                        normalized[i][j] = 0.0;   // 或 0.5，看你的需求
                    }
                }
            }
        }
    ...
    private static double[] minMaxNormalize(double[] fitness) {
        ...
        double max = -Double.MAX_VALUE;
            ...
            for (int j = 0; j < dim; j++) {
                ...
                if (val == Double.MAX_VALUE) {
                    normalized[j] = Double.MAX_VALUE;
                } else {
                    if (max > min) {
                        normalized[j] = (val - min) / (max - min);
                    } else {
                        normalized[j] = 0.0;   // 或 0.5，看你的需求
                    }
                }
            }
...
//                    estimatedFitness[i] = Math.max(1, dMin / threshold) * fitnessesForModel[index];
