...
public class GPRuleEvolutionStateLifelongGPV68 extends GPRuleEvolutionStateLifelongGPV3 {
    ...
    ArrayList<double[]> thresholdsEveryGeneration = new ArrayList<>();

    ArrayList<double[]> meanPCDistanceEveryGeneration = new ArrayList<>();

    ConvergenceChecker checker;

    boolean onlyCurrentTaskPhase = true;

    ArrayList<Integer> switchGen = new ArrayList<>();

    public double refFit;

    ArrayList<Individual> savedTopIndividuals;
...
//                if (onlyCurrentTaskPhase) {
                if (checker.check(meanFitnessCurrentTask) && onlyCurrentTaskPhase) {
                    ...
                    ((surrogateClearingMultitreeEvaluatorV68) evaluator).nonIntermediatePop = false;
                }
            }
            if(!onlyCurrentTaskPhase) {
...
/*            double[] fitnessesForModel = new double[population.subpops[0].individuals.length];
            ...
            for (int ind = 0; ind < population.subpops[0].individuals.length; ind++) {
                if (generation <= generationPerTask) {
                    ...
                    fitnessesForModel[ind] = (Double) population.subpops[0].individuals[ind].fitness.trials.get(population.subpops[0].individuals[ind].fitness.trials.size() - 1);
                }
            }
            ...
            surrogateFitness.add(fitnessesForModel);
            surrogateSamples.add(indsCharListsMultiTreeCopy); */
