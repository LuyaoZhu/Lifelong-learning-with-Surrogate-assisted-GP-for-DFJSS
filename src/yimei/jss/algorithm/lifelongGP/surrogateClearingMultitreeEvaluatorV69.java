...
public class surrogateClearingMultitreeEvaluatorV69 extends SimpleEvaluator {
...
//            multitreeClearingFirst.clearPopulation(state, radius, capacity, phenoCharacterisation);
            ...
            if (nonIntermediatePop) {
                ...
                if( min < ((GPRuleEvolutionStateLifelongGPV69) state).refFit) {
                    //then normalise
                    ((GPRuleEvolutionStateLifelongGPV69) state).normalisePopulation(min, ((GPRuleEvolutionStateLifelongGPV69) state).refFit);
                }
                ...
                if(!((GPRuleEvolutionStateLifelongGPV69) state).onlyCurrentTaskPhase) {
                    nonIntermediatePop = false;
                }

                if(state.generation == ((GPRuleEvolutionStateLifelongGPV69) state).generationPerTask - 1 ||
                        state.generation == ((GPRuleEvolutionStateLifelongGPV69) state).generationPerTask * 2 - 1 ||
                        state.generation == ((GPRuleEvolutionStateLifelongGPV69) state).generationPerTask * 3 - 1 ||
                        state.generation % ((GPRuleEvolutionStateLifelongGPV69) state).generationPerTask == 0 ) {
...
//                this.evaluatePopulation(state, tempindsCharListsMultiTree, tempfitnessesForModel);
                ...
                double[][] estimatedFitness = new double[state.generation/((GPRuleEvolutionStateLifelongGPV69) state).generationPerTask + 1][state.population.subpops[0].individuals.length];
                for (int task=0; task<estimatedFitness.length; task++) {
                    if (task == estimatedFitness.length - 1) {
                        estimatedFitness[task] = this.evaluatePopulation(state, tempindsCharListsMultiTree, tempfitnessesForModel, ((GPRuleEvolutionStateLifelongGPV69) state).surrogateThreshold);
                    } else {
                        for (int ind=0; ind < state.population.subpops[0].individuals.length; ind++) {
                            GPIndividual individual = (GPIndividual) state.population.subpops[0].individuals[ind];
                            double[] doublePC =  Arrays.stream(individual.PC).asDoubleStream().toArray();
                            estimatedFitness[task][ind] = ((GPRuleEvolutionStateLifelongGPV69) state).GPRSurrogateModels.get(task).estimate(doublePC);
                        }

//                        estimatedFitness[task] = this.evaluatePopulation(state, ((GPRuleEvolutionStateLifelongGPV69) state).surrogateSamples.get(task), ((GPRuleEvolutionStateLifelongGPV69) state).surrogateFitness.get(task), ((GPRuleEvolutionStateLifelongGPV69) state).surrogateThreshold);
                        //normalise
                ...
                double[] thresholds = ((GPRuleEvolutionStateLifelongGPV69) state).thresholdsEveryGeneration.get(((GPRuleEvolutionStateLifelongGPV69) state).thresholdsEveryGeneration.size()-1);
