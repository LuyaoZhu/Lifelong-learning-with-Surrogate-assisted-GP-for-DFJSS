 ...
 * add a confidence to the surrogate
 * adaptively change the thresholds
 *
 * @author luyao
 */

public class GPRuleEvolutionStateLifelongGPV100 extends GPRuleEvolutionStateLifelongGPV3 {
...
//        System.out.println(diversityValue[0]);
        ...
        if (generation >= generationPerTask) {
            ...
            for (int t = 0; t < surrogateSamples.size(); t++) {
                estimatedFitness[t] = evaluatePopulation(this, surrogateSamples.get(t), surrogateFitness.get(t), Double.MAX_VALUE);
...
//                    estimatedFitness[i] = Math.max(1, dMin / threshold) * fitnessesForModel[index];
                    double referenceDistance = PhenoCharacterisation.distance(indsCharListsMultiTree[index], referencePC);

                    if (dMin <= threshold) {
                        estimatedFitness[i] = fitnessesForModel[index] * Math.exp(dMin/referenceDistance);
                    } else {
                        estimatedFitness[i] = Double.MAX_VALUE;
                    }
