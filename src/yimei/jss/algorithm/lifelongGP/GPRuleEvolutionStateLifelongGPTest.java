...
public class GPRuleEvolutionStateLifelongGPTest extends GPRuleEvolutionStateLifelongGPV0 {
...
//        System.out.println(diversityValue[0]);
        ...
        int numTask =((MultipleTreeMultipleRuleEvaluationModel) problem.getEvaluationModel()).getSchedulingSet().getSimulations().size();
        double[][] fitnessOneTask = new double[numTask][this.population.subpops[0].individuals.length];
        double[][] result = new double[fitnessOneTask.length][fitnessOneTask.length];

        if(generation == 10) {

            for (int i = 0; i < numTask; i++) {
                DynamicSimulation sim = (DynamicSimulation) ((MultipleTreeMultipleRuleEvaluationModel) problem.getEvaluationModel()).getSchedulingSet().getSimulations().get(i);
                ...
                for (int ind = 0; ind < this.population.subpops[0].individuals.length; ind++) {
                    ...
                    sim.setSequencingRule(sequencingRule); //indicate different individuals
                    sim.setRoutingRule(routingRule);
                    ...
                    double ObjValue = sim.objectiveValue(objective); // this line: the value of makespan
                    ...
                    fitnessOneTask[i][ind] = ObjValue;
                }

            }

            SpearmansCorrelation sc = new SpearmansCorrelation();
            for (int i = 0; i < fitnessOneTask.length; i++) {
                result[i][i] = 1.0;
                for (int j = i + 1; j < fitnessOneTask.length; j++) {
                    double corr = sc.correlation(fitnessOneTask[i], fitnessOneTask[j]);
                    result[i][j] = corr;
                    result[j][i] = corr;
                }
            }
