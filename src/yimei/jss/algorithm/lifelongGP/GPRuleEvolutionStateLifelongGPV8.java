...
/**
 * in all generations of each task, generate 3*population, and collect individuals with good quality (fitness < reference rule's fitness) and unique PC
 * preselection + remove duplicates
 *
 *
 * @author luyao
 */

public class GPRuleEvolutionStateLifelongGPV8 extends GPRuleEvolutionStateLifelongGPV3 {
...
// -------------------------------
            ...
            SimpleKMedoids.Result r = SimpleKMedoids.fit(PCs, population.subpops[0].individuals.length, 20160608, 18);
...
//            writeFirstSelectedCasesNumToFile(firstSelectedCasesNum);
        ...
        if(generation == generationPerTask-1 || generation == generationPerTask*2-1  || generation == generationPerTask*3-1 ){
            ...
            for (int sub = 0; sub < this.population.subpops.length; sub++) {
                for (int replace = 0; replace<savedIndividuals.size(); replace++) {
                    population.subpops[sub].individuals[replace] = savedIndividuals.get(replace);
                }
            }
