 ...
 */
public class SameIndexCrossoverPipelineInvestigation extends CrossoverPipeline {
    /**
...

    {
        ...
        for (int q = start; q < n + start; /* no increment */)  // keep on going until we're filled up
        {
            ...
            //do an investigation, the fitness of offspring, compared with parents.
            ...
            AbstractRule sequencingRule = new GPRule(RuleType.SEQUENCING, j1.trees[0]);
            AbstractRule routingRule = new GPRule(RuleType.ROUTING, j1.trees[1]);
            simulation.setSequencingRule(sequencingRule);
            simulation.setRoutingRule(routingRule);
            simulation.run();
            Objective[0] = simulation.objectiveValue(objective);
            ((MultiObjectiveFitness)j1.fitness).setObjectives(state,Objective);
            simulation.reset();

            double[] Objective1 = new double[1];
            sequencingRule = new GPRule(RuleType.SEQUENCING, j2.trees[0]);
            routingRule = new GPRule(RuleType.ROUTING, j2.trees[1]);
            simulation.setSequencingRule(sequencingRule);
            simulation.setRoutingRule(routingRule);
            simulation.run();
            Objective1[0]= simulation.objectiveValue(objective);
            ((MultiObjectiveFitness)j2.fitness).setObjectives(state,Objective1);
            simulation.reset();

            ((GPRuleEvolutionState)state).sumNumCrossover++;

            if(j1.fitness.fitness() < parents[0].fitness.fitness() || j2.fitness.fitness() < parents[1].fitness.fitness())  {
                ((GPRuleEvolutionState)state).numberImprovedCrossover++;
            }

            //then if fitness improved, analyse the correlation between subtrees and both parents

            //1.e.g., the correlation between cases
