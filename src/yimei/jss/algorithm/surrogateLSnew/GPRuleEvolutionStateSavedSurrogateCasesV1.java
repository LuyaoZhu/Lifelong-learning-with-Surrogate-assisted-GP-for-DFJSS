...
import yimei.jss.algorithm.surrogateCaseLS.KneePointFinder;
...
import static yimei.jss.algorithm.surrogateCaseLS.SpearmanClustering.agglomerativeClustering;
import static yimei.jss.algorithm.surrogateCaseLS.SpearmanClustering.computeSpearmanDistanceMatrix;
...
public class GPRuleEvolutionStateSavedSurrogateCasesV1 extends GPRuleEvolutionStateSavedSurrogateCases {

    public ArrayList<Double> PCDistanceThreshold = new ArrayList<>();
    ...
    public int evolve() {
        ...
        uniquePCNum.add(surrogatePCEveryGen.get(generation).length);
        ...
        if (generation >= switchGen) {
            ...
            //select cases
            double pcDistance = selectCases();
            PCDistanceThreshold.add(pcDistance);
            // 2. assign fitness
            assignFitness((MultipleTreeMultipleRuleEvaluationModel)problem.getEvaluationModel());
        } else {
            PCDistanceThreshold.add(0.0);
        }
...
// SHOULD WE QUIT?
        ...
        if (generation == numGenerations - 1) {
            ...
            writeDiversityToFile(surrogateClearingMultitreeEvaluatorV1.entropyDiversity);
            ...
            writePCDistanceToFile(PCDistanceThreshold);
...
// INCREMENT GENERATION AND CHECKPOINT
    ...
    private void assignFitness(MultipleTreeMultipleRuleEvaluationModel model) {

        int realEvaluationNum = 0;
        ...
        for (int instance = 0; instance < surrogatePCEveryGen.size() - 1; instance++) {
            ...
            if (finalSelectedCasesIndex.contains(instance)) {
                ...
                for (int ind=0; ind< population.subpops[0].individuals.length; ind++) {
                    int[] estimatedPC = ((GPIndividual)population.subpops[0].individuals[ind]).PC;
                    ...
                    //it would be better to add a confidence interval
                    if (dMin > pcDistance) { //use real evaluation to replace the estimation
                        GPIndividual indi = (GPIndividual) population.subpops[0].individuals[ind];
                        ...
                        ((GPIndividual) population.subpops[0].individuals[ind]).caseFitness.set(instance, ObjValue);
                        ((GPIndividual) population.subpops[0].individuals[ind]).realCaseFitness.set(instance, ObjValue);

                        //in addition, add it to the surrogate, update the surrogate
                        int[][] extendedPC = Arrays.copyOf(surrogatePCEveryGen.get(instance), surrogatePCEveryGen.get(instance).length + 1);
                        extendedPC[surrogatePCEveryGen.get(instance).length] = estimatedPC;
                        double[] extendedFitness = Arrays.copyOf(surrogateFitnessEveryGen.get(instance), surrogateFitnessEveryGen.get(instance).length + 1);
                        extendedFitness[surrogateFitnessEveryGen.get(instance).length] = ObjValue;
                        surrogatePCEveryGen.set(instance, extendedPC);
                        surrogateFitnessEveryGen.set(instance, extendedFitness);
                        realEvaluationNum++;
                    } else {
                            ((GPIndividual) population.subpops[0].individuals[ind]).caseFitness.set(instance, surrogateFitnessEveryGen.get(instance)[index]);
//                            ((GPIndividual) population.subpops[0].individuals[ind]).realCaseFitness.set(instance, surrogateFitnessEveryGen.get(instance)[index]);
                    }
                }
            }
        }
        System.out.println("The number of extral real evaluations is " + realEvaluationNum);
        extraEvaluateNum.add(realEvaluationNum);
        System.out.println("The number of evaluations is " + (realEvaluationNum+population.subpops[0].individuals.length));
    ...
    private double selectCases() {

        Map<Integer, List<Integer>> indToMatchingInstances = new HashMap<>();
        ArrayList<Double> averagePCDistance = new ArrayList<>();

        //1. determine the cases
        for (int instance = 0; instance < surrogatePCEveryGen.size() - 1; instance++) {
            double sumPCDistance = 0;
            int samePCNum = 0;
            for (int ind=0; ind< population.subpops[0].individuals.length; ind++) {
                int[] estimatedPC = ((GPIndividual)population.subpops[0].individuals[ind]).PC;
                double dMin = Double.MAX_VALUE;
                ...
                if (dMin <= 0) {
                    samePCNum++;
                    indToMatchingInstances.computeIfAbsent(ind, k -> new ArrayList<>()).add(instance);
                }
                ((GPIndividual) population.subpops[0].individuals[ind]).caseFitness.add(surrogateFitnessEveryGen.get(instance)[index]);
            }
            averagePCDistance.add(sumPCDistance/population.subpops[0].individuals.length);
            samePCNumInCases.add(samePCNum);
        }

        //2.use knee-point to select cases
/*        KneePointFinder.KneeResult result = KneePointFinder.findKneePoint(samePCNumInCases, 5);
        // find all the point that
        System.out.print("The index of selected cases are ");
        for (int i = 5; i < samePCNumInCases.size(); i++) {
            if (samePCNumInCases.get(i) > result.kneeValue) {
                firstSelectedCasesIndex.add(i);
                System.out.print(i + ", ");
            }
        }*/

        KneePointFinder.KneeResult result = KneePointFinder.findKneePoint(averagePCDistance, 5);
        System.out.print("The index of selected cases are ");
        for (int i = 5; i < averagePCDistance.size(); i++) {
        ...
        //use k-means to select cases
//            firstSelectedCasesIndex.add(generation);
        if(firstSelectedCasesIndex.size()> 1500){

            Map<Integer, List<Double>> indToFitnesses = new HashMap<>();
            for (Map.Entry<Integer, List<Integer>> entry : indToMatchingInstances.entrySet()) {
                int ind = entry.getKey();
                List<Integer> matchedInstances = entry.getValue();

                if (matchedInstances.containsAll(firstSelectedCasesIndex)) {
                    // 该个体在所有 selectedInstances 上都PC匹配
                    List<Double> fitnessList = new ArrayList<>();
                    GPIndividual individual = (GPIndividual) this.population.subpops[0].individuals[ind];
                    for (int instance : firstSelectedCasesIndex) {
                        fitnessList.add(individual.caseFitness.get(instance));
                    }
                    indToFitnesses.put(ind, fitnessList);
                }
            }

            // select cases based on diversity
            double[][] fitnessMatrix = new double[firstSelectedCasesIndex.size()][indToFitnesses.size()];
            int num=0;
            for (int i : indToFitnesses.keySet()) {
                List<Double> fitnessList = indToFitnesses.get(i);
                for (int j = 0; j < fitnessList.size(); j++) {
                    fitnessMatrix[j][num] = fitnessList.get(j);
                }
                num++;
            }

            double[][] distanceMatrix = computeSpearmanDistanceMatrix(fitnessMatrix);
            Map<Integer, List<Integer>> clusters = agglomerativeClustering(distanceMatrix, 15);

            for (Map.Entry<Integer, List<Integer>> entry : clusters.entrySet()) {
                finalSelectedCasesIndex.add(firstSelectedCasesIndex.get(entry.getValue().get(0)));
            }
...
//            terminalsFrom[subPopNum] = subPop1TerminalSet;
    ...
    public void writePCDistanceToFile(ArrayList<Double> PCDistance) {
        // fzhang 2019.5.21 save the number of cleared individuals
        File weightFile = new File(out_dir + "/job." + jobSeed + ".pcDistanceThreshold.csv"); // jobSeed = 0
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter(weightFile));

            // Dynamically create the header based on the size of entropyDiversity elements
            StringBuilder header = new StringBuilder("Gen");
            if (!PCDistance.isEmpty()) {
                for (int j = 0; j < 1; j++) {
                    header.append(",PCDistance").append(j);
                }
            }
            writer.write(header.toString());
            writer.newLine();
            ...
            for (int i = 0; i < PCDistance.size(); i++) {
                StringBuilder line = new StringBuilder(i + switchGen + "");
                line.append(", ").append(PCDistance.get(i));
                writer.write(line.toString());
                writer.newLine();
            }
