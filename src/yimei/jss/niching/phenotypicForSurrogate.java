...
public class phenotypicForSurrogate {
    ...
    static int[][][] oneIndCharListsRanks = null; //save the PC information
    static int[][] oneIndCharListsMultiTreeRanks = null;
   ...
   /* public static double[][] phenotypicPopulation(final EvolutionState state,
    ...
    public static double[][] phenotypicPopulation(final EvolutionState state,
        ...
        RuleType[] ruleTypes = {RuleType.SEQUENCING, RuleType.ROUTING}; //ruleType is an array
        numTrees = state.parameters.getIntWithDefault(new Parameter(P_TREESIZE), null, 2);
...
/*          if(nonIntermediatePop){
    ...
    //2021.7.26 when calculating the phenotypic characterisation of the inds in the whole population, use this one.
    public static int[][][] muchBetterPhenotypicPopulationRanks(EvolutionState state, PhenoCharacterisation[] pc)
    {
        Individual[] inds = state.population.subpops[0].individuals;
        int[][][] indsCharListsMultiTree = new int[inds.length][][];
        for(int i = 0; i < inds.length; i++)
            indsCharListsMultiTree[i] = phenotypicPopulationRanks(state, pc, inds[i]);

        return indsCharListsMultiTree;
    }

    public static int[][] phenotypicPopulationRanks(final EvolutionState state,
                                             PhenoCharacterisation[] pc,
                                             Individual ind) {

        RuleType[] ruleTypes = {RuleType.SEQUENCING, RuleType.ROUTING}; //ruleType is an array
        int numTrees = ((GPIndividual)ind).trees.length;
        oneIndCharListsRanks = new int[numTrees][][];
        ...
        for (int treeID = 0; treeID < numTrees; treeID++)
        {
            ...
            oneIndCharListsRanks[treeID] = phenoCharacterisation.characteriseRanks(new GPRule(ruleType, ((GPIndividual) ind).trees[treeID]));
        }

        //combine the phenotype of sequencing rule with the phenotype of routing rule
        oneIndCharListsMultiTreeRanks = Arrays.stream(oneIndCharListsRanks).flatMap(Arrays::stream).toArray(int[][]::new);
        return oneIndCharListsMultiTreeRanks;
    }
