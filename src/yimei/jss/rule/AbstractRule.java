...
//				fitnesses[i] += normObjValue;
                ...
                if(simulation.getSystemState().getClockTime() == Double.MAX_VALUE) {
                    if(Simulation.optimisationObjective.endsWith("profit")){
                        ObjValue = -3000;
                    }
                    else {
                    ObjValue = 3000;
...
//					fitnesses[i] += normObjValue;
    ...
    public void calcFitness(Fitness fitness, Fitness normFitness, EvolutionState state, SchedulingSet schedulingSet, AbstractRule otherRule,
                            List<Objective> objectives) {
        // whenever fitness is calculated, need a routing rule and a sequencing rule
        if (this.getType() == otherRule.getType()) {
            System.out.println(
        ...
        double[] fitnesses = new double[objectives.size()];
        double[] normFitnesses = new double[objectives.size()];
...
//				fitnesses[i] += normObjValue;
                ...
                if(simulation.getSystemState().getClockTime() == Double.MAX_VALUE) {
                    if(Simulation.optimisationObjective.endsWith("profit")){
                        ObjValue = -schedulingSet.getObjectiveLowerBound(i, col) * 2;
                    }
                    else {
                        ObjValue = schedulingSet.getObjectiveLowerBound(i, col) * 2;
                    }
                ...
                double normObjValue = Math.min(2, ObjValue / schedulingSet.getObjectiveLowerBound(i, col));

                normFitnesses[i] += normObjValue;
            }
...
//					fitnesses[i] += normObjValue;
        ...
        MultiObjectiveFitness normF = (MultiObjectiveFitness) normFitness;
        normF.setObjectives(state, normFitnesses);
    }
...
//==================================================end=======================================================
    ...
    public void calcFitness(Fitness fitness, EvolutionState state, SchedulingSet schedulingSet, AbstractRule otherRule1,  AbstractRule otherRule2,
                            List<Objective> objectives) {
        // whenever fitness is calculated, need a routing rule and a sequencing rule
        if (this.getType() == otherRule1.getType() || this.getType() == otherRule2.getType()) {
            System.out.println(
                    "We need one routing rule, one sequencing rule and one ordering rule, rule type is incorrect.");
            return;
        }
        AbstractRule routingRule = null;
        AbstractRule sequencingRule = null;
        AbstractRule orderingRule = null;
        // check type, not here
        if (this.getType() == RuleType.ROUTING) {
            routingRule = this;

            if(otherRule1.getType() == RuleType.SEQUENCING) {
                sequencingRule = otherRule1;
                orderingRule = otherRule2;
            }else {
                sequencingRule = otherRule2;
                orderingRule = otherRule1;
            }

        } else if (this.getType() == RuleType.SEQUENCING) {
            sequencingRule = this;

            if(otherRule1.getType() == RuleType.ROUTING) {
                routingRule = otherRule1;
                orderingRule = otherRule2;
            }else {
                routingRule = otherRule2;
                orderingRule = otherRule1;
            }
        } else if (this.getType() == RuleType.ORDERING) {
            orderingRule = this;
        ...
        for (int j = 0; j < simulations.size(); j++) {
            ...
            Simulation.optimisationObjective = objectives.get(0).getName();
...
//				fitnesses[i] += normObjValue;
                ...
                if(ObjValue >= Double.POSITIVE_INFINITY ||  ObjValue <= Double.NEGATIVE_INFINITY || Double.isNaN(ObjValue)){
                    if(Simulation.optimisationObjective.endsWith("profit")){
                        ObjValue = -3000;
                    }
                    else {
...
//					fitnesses[i] += normObjValue;
    ...
    //---------------------------------use ordering rule to priority pairs-----------------------------------
    public OperationOption nextPairOption(OrderingDecisionSituation orderingDecisionSituation) {

        List<OperationOption> queue = orderingDecisionSituation.getQueue();
        SystemState systemState = orderingDecisionSituation.getSystemState();
        //================original=================
        //==================start==================
        OperationOption bestOperationOption = queue.get(0);
        bestOperationOption.setPriority(priority(bestOperationOption, bestOperationOption.getWorkCenter(), systemState));
        // loop all the options, save the best one as "selected" one
        for (int i = 1; i < queue.size(); i++) {
            OperationOption operationOption = queue.get(i);
            operationOption.setPriority(priority(operationOption, operationOption.getWorkCenter(), systemState));
    ...
    public Job nextJob(OrderingDecisionSituation orderingDecisionSituation, AbstractRule orderingRule) {
        ...
        bestJob.setPriority(priorityJob(bestJob, systemState, orderingRule));
        // loop all the options, save the best one as "selected" one
        for (int i = 1; i < queue.size(); i++) {
            Job job = queue.get(i);
            job.setPriority(priorityJob(job, systemState, orderingRule));
    ...
    public double priorityJob(Job job,
